...
// 변형 타입 설정
const VARIANT_TYPES = [
  { id:"gender_f",   label:{ko:"여성형",       en:"Feminine"},        prompt:{ko:"여성형은?",      en:"Feminine form?"} },
  { id:"gender_m",   label:{ko:"남성형",       en:"Masculine"},       prompt:{ko:"남성형은?",      en:"Masculine form?"} },
  { id:"plural_m",   label:{ko:"복수 남성형",  en:"Plural (M)"},      prompt:{ko:"복수 남성형은?", en:"Plural masculine?"} },
  { id:"plural_f",   label:{ko:"복수 여성형",  en:"Plural (F)"},      prompt:{ko:"복수 여성형은?", en:"Plural feminine?"} },
  { id:"verb_1s",    label:{ko:"1인칭 단수",   en:"1st sg."},         prompt:{ko:"나는? (1인칭 단수)", en:"I (1st sg.)?"} },
  { id:"verb_2ms",   label:{ko:"2인칭 남단수", en:"2nd sg. M"},       prompt:{ko:"너는? (남성)", en:"You (M, sg.)?"} },
  { id:"verb_2fs",   label:{ko:"2인칭 여단수", en:"2nd sg. F"},       prompt:{ko:"너는? (여성)", en:"You (F, sg.)?"} },
  { id:"verb_3ms",   label:{ko:"3인칭 남단수", en:"3rd sg. M"},       prompt:{ko:"그는?",         en:"He?"} },
  { id:"verb_3fs",   label:{ko:"3인칭 여단수", en:"3rd sg. F"},       prompt:{ko:"그녀는?",       en:"She?"} },
  { id:"verb_1p",    label:{ko:"1인칭 복수",   en:"1st pl."},         prompt:{ko:"우리는?",       en:"We?"} },
  { id:"verb_2mp",   label:{ko:"2인칭 남복수", en:"2nd pl. M"},       prompt:{ko:"너희는? (남성)", en:"You (M, pl.)?"} },
  { id:"verb_2fp",   label:{ko:"2인칭 여복수", en:"2nd pl. F"},       prompt:{ko:"너희는? (여성)", en:"You (F, pl.)?"} },
  { id:"verb_3mp",   label:{ko:"3인칭 남복수", en:"3rd pl. M"},       prompt:{ko:"그들은? (남성)", en:"They (M)?"} },
  { id:"verb_3fp",   label:{ko:"3인칭 여복수", en:"3rd pl. F"},       prompt:{ko:"그들은? (여성)", en:"They (F)?"} },
  { id:"poss_1s",    label:{ko:"소유 1인칭",   en:"Poss. 1st"},       prompt:{ko:"나의 ~는?",     en:"My ~?"} },
  { id:"poss_2ms",   label:{ko:"소유 2인칭(남)", en:"Poss. 2M"},      prompt:{ko:"너의 ~ (남)?",  en:"Your (M) ~?"} },
  { id:"poss_2fs",   label:{ko:"소유 2인칭(여)", en:"Poss. 2F"},      prompt:{ko:"너의 ~ (여)?",  en:"Your (F) ~?"} },
  { id:"poss_3ms",   label:{ko:"소유 3인칭(남)", en:"Poss. 3M"},      prompt:{ko:"그의 ~는?",     en:"His ~?"} },
  { id:"poss_3fs",   label:{ko:"소유 3인칭(여)", en:"Poss. 3F"},      prompt:{ko:"그녀의 ~는?",   en:"Her ~?"} },
];
...
export default function HebrewQuiz() {
  ...
  const addNewWordFromPealim=()=>{
    ...
    setShowPealimModal(false); setPealimPreview(null); setPealimRoot(""); setPealimResults([]);
    showToast(`✅ "${pealimPreview.infinitive}" 단어와 ${variants.length}개 변형이 추가됐어요!`);
  };
                  ...
                  {words.filter(w=>w.wordType==="verb"||!w.wordType).length>0&&(
                    <div>
                      ...
                      <div style={{display:"flex",flexDirection:"column",gap:"4px",maxHeight:"120px",overflowY:"auto"}}>
                        {words.filter(w=>w.wordType==="verb"||!w.wordType).slice(0,8).map(w=>(
                          <button key={w.id} onClick={()=>importPealimToWord(w.id)}
                            style={{padding:"7px 12px",borderRadius:"8px",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",color:"#c8c6d8",cursor:"pointer",textAlign:"left",fontSize:"0.85rem",display:"flex",gap:"8px",alignItems:"center"}}>
                            <span style={{fontFamily:"Arial",direction:"rtl",color:"#c4a050"}}>{w.hebrew}</span>
                            <span style={{color:"#5a5870"}}>{w.meaning}</span>
                          </button>
                        ))}
                      </div>
                    </div>
                ...
                <div style={{height:"52px",marginBottom:"8px",display:"flex",alignItems:"center",justifyContent:"center"}}>
                  {lastResult?.correct
                    ...
                    :<div style={{...S.feedbackWrong,padding:"8px 14px",fontSize:"0.88rem"}}>
                      ...
                      <SpeakBtn text={vq.answer} onSpeak={speak} muted={muted}/>
                    </div>}
                </div>
              )}
              <div className="quiz-btn-row" style={S.quizBtnRow}>
                ...
                return(
                  <div key={i} style={{...S.wrongItem,flexDirection:"column",alignItems:"flex-start",gap:"3px"}}>
                    <div style={{display:"flex",alignItems:"center",gap:"8px",width:"100%"}}>
                      ...
                      <SpeakBtn text={r.answer} onSpeak={speak} muted={muted}/>
                    </div>
