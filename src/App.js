...
export default function App() {
  ...
  // ── RENDER ────────────────────────────────────────────────
  // ── Render helpers ────────────────────────────────────────
  function renderWalletPickModal() {
    if (!walletPickWord) return null;
    return (
      <div style={{...Mo.overlay,zIndex:9999}} onClick={()=>setWalletPickWord(null)}>
        <div style={{...Mo.box,maxWidth:"300px"}} onClick={e=>e.stopPropagation()}>
          <h3 style={Mo.title}>단어장 선택</h3>
          <div style={{display:"flex",flexDirection:"column",gap:"4px",marginBottom:"12px"}}>
            {wallets.map(wl=>{
              const inW=wl.wordIds.includes(walletPickWord);
              const bc=inW?"rgba(232,74,95,0.28)":TL;
              const db=inW?wl.color:"rgba(255,255,255,0.2)";
              return (
                <button key={wl.id} onClick={()=>toggleWordInWallet(wl.id,walletPickWord)}
                  style={{display:"flex",alignItems:"center",gap:"9px",padding:"9px 12px",
                  borderRadius:"2px",background:inW?"rgba(232,74,95,0.07)":"rgba(255,255,255,0.02)",
                  border:"1px solid "+bc,cursor:"pointer",textAlign:"left"}}>
                  <div style={{width:"13px",height:"13px",borderRadius:"2px",flexShrink:0,
                    background:inW?wl.color:"transparent",border:"1px solid "+db,
  ...
  function renderBulkWalletMenu() {
    if (!selectedIds.size) return null;
    const sel = bulkWalletOpen instanceof Set ? bulkWalletOpen : new Set();
    return (
      <>
        <div style={{position:"relative"}}>
          <button onClick={()=>setBulkWalletOpen(v=>!v)}
            style={{...Bt.ghost,fontSize:"0.62rem",padding:"2px 7px",color:TH}}>
            {"📚 "+selectedIds.size+"개 ▾"}
          </button>
          {bulkWalletOpen&&(
            <div style={{position:"absolute",top:"100%",left:0,zIndex:50,marginTop:"2px",
              background:TS,border:"1px solid "+TL,borderRadius:"2px",padding:"8px",
              minWidth:"155px",boxShadow:"0 8px 24px rgba(0,0,0,0.6)"}}>
              {wallets.map(wl=>{
                const checked=sel.has(wl.id);
                return (
                  <button key={wl.id} onClick={()=>{
                    const ns=new Set(sel);
                    checked?ns.delete(wl.id):ns.add(wl.id);
                    setBulkWalletOpen(ns);
                  }} style={{display:"flex",alignItems:"center",gap:"7px",padding:"5px 7px",
                    width:"100%",background:"transparent",border:"none",borderRadius:"2px",
                    cursor:"pointer",color:checked?wl.color:TT,fontSize:"0.78rem",marginBottom:"1px"}}>
                    <span style={{width:"11px",height:"11px",borderRadius:"2px",flexShrink:0,
                      background:checked?wl.color:"transparent",
                      border:"1px solid "+(checked?wl.color:"rgba(255,255,255,0.2)"),
                      display:"flex",alignItems:"center",justifyContent:"center"}}>
                      {checked&&<span style={{color:"#fff",fontSize:"0.5rem"}}>✓</span>}
                    </span>
                    {wl.name}
                  </button>
                );
              ...
              {sel.size>0&&(
                <button onClick={()=>{
                  saveWallets(wallets.map(wl=>sel.has(wl.id)?{...wl,wordIds:[...new Set([...wl.wordIds,...selectedIds])]}:wl));
                  setBulkWalletOpen(false);
                  setSelectedIds(new Set());
                  showToast(selectedIds.size+"개 추가!");
                }} style={{...Bt.primary,width:"100%",marginTop:"4px",padding:"5px",fontSize:"0.75rem"}}>
                  {sel.size+"개 단어장에 추가"}
                </button>
              )}
            </div>
        ...
        <button onClick={()=>{
          if(window.confirm(selectedIds.size+"개 삭제할까요?")){
            setWords(ws=>ws.filter(w=>!selectedIds.has(w.id)));
            setSelectedIds(new Set());
          }
        }} style={{...Bt.ghost,fontSize:"0.62rem",padding:"2px 7px",color:TA,borderColor:"rgba(232,74,95,0.28)"}}>
          ...
          {activeTab==="list"&&<div>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:"1px solid "+(TL)}}>
              <div style={{display:"flex",alignItems:"center",gap:"7px",flexWrap:"wrap"}}>
                ...
                {selectedIds.size>0&&renderBulkWalletMenu()}
              </div>
            </div>
