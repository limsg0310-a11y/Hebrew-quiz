...
const QUIZ_TYPES = {
  HEB_TO_MEAN: "heb_to_mean",
  MEAN_TO_HEB: "mean_to_heb",
  MIXED: "mixed",
};
const QUIZ_FILTERS = {
  ALL: "all",
...
const STATUS_CONFIG = {
  ...
  hard: {
    ...
    emoji: "🔥",
    color: "#f07050",
    bg: "rgba(200,80,60,0.15)",
    border: "rgba(200,80,60,0.35)",
  },
};

// 닉쿠드(모음기호) 제거 — TTS 발음 정확도 향상
...
function generateQuestion(word, allWords, type) {
  const actualType =
    type === QUIZ_TYPES.MIXED
      ? Math.random() > 0.5
        ? QUIZ_TYPES.HEB_TO_MEAN
        : QUIZ_TYPES.MEAN_TO_HEB
      : type;
  const question =
    actualType === QUIZ_TYPES.HEB_TO_MEAN ? word.hebrew : word.meaning;
  const answer =
    actualType === QUIZ_TYPES.HEB_TO_MEAN ? word.meaning : word.hebrew;
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius: "8px",
        cursor: "pointer",
        padding: size === "lg" ? "10px 16px" : "6px 10px",
...
export default function HebrewQuiz() {
  // API Key — reads from CodeSandbox env var
  const envKey = process.env.REACT_APP_GOOGLE_TTS_KEY || "";
  const [apiKey] = useState(envKey);
  const [ttsReady] = useState(!!envKey);

  const speak = useCallback(
    async (text) => {
      if (apiKey) {
        try {
          await googleTTS(text, apiKey);
          return;
        } catch {}
      }
      ...
      ws.map((w) =>
        w.id === id
          ? {
              ...w,
              status,
              streak: status === "mastered" ? 3 : 0,
              wrongCount: status === "hard" ? 2 : 0,
            }
          : w
      )
    );
  ...
  const exportWords = () => {
    ...
    a.href = url;
    a.download = `hebrew-vocab-${new Date().toISOString().slice(0, 10)}.json`;
      ...
      try {
        ...
        const imported = raw
          .map((w) => ({
            ...
            status: ["learning", "mastered", "hard"].includes(w.status)
              ? w.status
              : "learning",
            streak: w.streak || 0,
        ...
        if (!imported.length) {
          showToast("불러올 단어가 없어요.", "err");
          return;
        }
        setImportPreview({ words: imported, fileName: file.name });
      } catch {
        showToast("파일을 읽을 수 없어요.", "err");
      }
    };
  ...
  const confirmImport = (merge) => {
    ...
    if (merge) {
      ...
      showToast(
        `📥 ${newOnes.length}개 추가! (중복 ${
          importPreview.words.length - newOnes.length
        }개 제외)`
      );
    } else {
  ...
  const getPool = () => {
    if (quizFilter === QUIZ_FILTERS.EXCLUDE_MASTERED)
      return words.filter((w) => w.status !== "mastered");
    if (quizFilter === QUIZ_FILTERS.HARD_ONLY)
      return words.filter((w) => w.status === "hard");
    return words;
  };
  ...
  const startQuiz = () => {
    ...
    const count = Math.min(
      quizCount === 9999 ? pool.length : quizCount,
      pool.length
    );
    const qs = shuffle(pool)
      .slice(0, count)
      .map((w) => generateQuestion(w, words, quizType));
    setQuestions(qs);
    setCurrent(0);
    setSelected(null);
    setConfirmed(false);
  ...
  const handleSelect = (choice) => {
    if (!confirmed) setSelected(choice);
  };
  const handleConfirm = () => {
    if (!selected) return;
    const correct = selected === questions[current].answer;
    if (correct) setScore((s) => s + 1);
    else setWrongWords((w) => [...w, questions[current]]);
    updateWordStats(questions[current].wordId, correct);
    setConfirmed(true);
    const q = questions[current];
    const hebText =
      q.questionType === QUIZ_TYPES.HEB_TO_MEAN ? q.question : q.answer;
  ...
  const addWord = () => {
    ...
    if (editId !== null) {
      setWords((ws) =>
        ws.map((w) =>
          w.id === editId
            ? { ...w, hebrew: newHebrew.trim(), meaning: newMeaning.trim() }
            : w
        )
      );
      setEditId(null);
  ...
  const filteredWords =
    listFilter === "all" ? words : words.filter((w) => w.status === listFilter);
  const q = questions[current];
  const progress =
    questions.length > 0
      ? ((current + (confirmed ? 1 : 0)) / questions.length) * 100
      : 0;
  const poolSize = getPool().length;
  const countOptions = [5, 10, 20, "전체"].map((v) => ({
    label: v === "전체" ? "전체" : `${v}문제`,
    value: v === "전체" ? 9999 : v,
  }));
  ...
  return (
    <div style={S.root}>
      ...
      {toast && (
        <div
          style={{ ...S.toast, ...(toast.type === "err" ? S.toastErr : {}) }}
        >
          {toast.msg}
        </div>
      )}
      ...
      {importPreview && (
        <div style={S.modalOverlay}>
          <div style={S.modal}>
            ...
            <p style={S.modalSub}>
              파일:{" "}
              <span style={{ color: "#c4a050" }}>{importPreview.fileName}</span>
            </p>
            <p style={S.modalSub}>
              <b style={{ color: "#e8e6f0" }}>{importPreview.words.length}개</b>{" "}
              단어 발견
            </p>
            <div style={S.modalPreview}>
              ...
              {importPreview.words.length > 5 && (
                ...
                >
                  ...외 {importPreview.words.length - 5}개
                </p>
              )}
            </div>
            <div className="modal-btn-row" style={S.modalBtnRow}>
              <button style={S.btnMerge} onClick={() => confirmImport(true)}>
                ➕ 현재에 추가
              </button>
              <button style={S.btnReplace} onClick={() => confirmImport(false)}>
                🔄 전체 교체
              </button>
              <button
                style={S.btnCancel2}
      ...
      <div style={S.container}>
        ...
        {!ttsReady && (
          <div style={S.noticeBanner}>
            💡 <b>발음 품질 개선 팁:</b> CodeSandbox에서 환경변수{" "}
            <code>REACT_APP_GOOGLE_TTS_KEY</code>를 설정하면 Google 고품질
            발음으로 업그레이드돼요!
          </div>
        )}
        ...
        {mode === MODES.LIST && (
          <div>
            <div style={S.card}>
              ...
              <div className="form-row" style={S.formRow}>
                ...
                <div style={{ display: "flex", gap: "8px" }}>
                  <button style={{ ...S.btnAdd, flex: 1 }} onClick={addWord}>
            ...
            <div className="io-bar" style={S.ioBar}>
              <div style={S.ioLeft}>
                <span style={S.ioTitle}>💾 단어장 저장 / 불러오기</span>
                <span style={S.ioSub}>
                  앱이 바뀌어도 파일로 유지할 수 있어요
                </span>
              </div>
              <div style={S.ioBtns}>
                <button style={S.btnExport} onClick={exportWords}>
                  ⬇️ 내보내기
                </button>
                <button
                  style={S.btnImportBtn}
            ...
            <div style={S.filterTabs}>
              {[
                ...
                <button
                  ...
                  style={{
                    ...S.filterTab,
                    ...(listFilter === val ? S.filterTabActive : {}),
                  }}
            ...
            <div style={S.wordList}>
              {filteredWords.length === 0 && (
                <div style={S.emptyMsg}>해당 상태의 단어가 없어요</div>
              )}
              {filteredWords.map((w, i) => {
                ...
                return (
                  <div
                    key={w.id}
                    style={{ ...S.wordItem, borderColor: st.border }}
                  >
                    ...
                    <div style={S.wordRight}>
                        ...
                        {["learning", "hard", "mastered"].map((s) => {
                          ...
                          return (
                            <button
                              ...
                              onClick={() => setManualStatus(w.id, s)}
                            >
                              {sc.emoji}
                            </button>
                          );
                      ...
                      <div style={S.wordActions}>
                        <button style={S.btnEdit} onClick={() => startEdit(w)}>
                          ✏️
                        </button>
                        <button
                          style={S.btnDel}
            ...
            <div style={S.card}>
              ...
              <div style={S.optionRow}>
                {[
                  [QUIZ_TYPES.HEB_TO_MEAN, "히브리어 → 뜻"],
                  [QUIZ_TYPES.MEAN_TO_HEB, "뜻 → 히브리어"],
                  [QUIZ_TYPES.MIXED, "랜덤 혼합"],
                ].map(([val, label]) => (
                  <button
                    key={val}
                    style={{
                      ...S.optBtn,
                      ...(quizType === val ? S.optBtnActive : {}),
                    }}
              ...
              <div style={S.optionRow}>
                {[
                  [QUIZ_FILTERS.ALL, `전체 (${words.length})`],
                  [
                    QUIZ_FILTERS.EXCLUDE_MASTERED,
                    `암기 제외 (${
                      words.filter((w) => w.status !== "mastered").length
                    })`,
                  ],
                  [QUIZ_FILTERS.HARD_ONLY, `🔥 어려운 것만 (${hardCount})`],
                ].map(([val, label]) => (
                  <button
                    key={val}
                    style={{
                      ...S.optBtn,
                      ...(quizFilter === val ? S.optBtnActive : {}),
                    }}
                    onClick={() => setQuizFilter(val)}
                  >
                    {label}
                  </button>
                ))}
              </div>
              <p style={S.settingLabel}>
                문제 수{" "}
                <span
                  style={{
                    color: "#5a5870",
                ...
                {countOptions.map(({ label, value }) => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...S.optBtn,
                        ...(quizCount === value ? S.optBtnActive : {}),
                        ...(isDisabled
                          ? { opacity: 0.3, cursor: "not-allowed" }
                          : {}),
                      }}
                      onClick={() => !isDisabled && setQuizCount(value)}
                      disabled={isDisabled}
                    >
                      {label}
                    </button>
              ...
              <div style={S.sliderWrap}>
                ...
                <input
                  type="range"
                  min={4}
                  max={Math.max(4, poolSize)}
                  value={Math.min(
                    quizCount === 9999 ? poolSize : quizCount,
                    poolSize
                  )}
              ...
              <div style={S.autoPlayRow}>
                ...
                <button
                  ...
                  style={{
                    ...S.toggleBtn,
                    ...(autoPlay ? S.toggleOn : S.toggleOff),
                  }}
                >
                  {autoPlay ? "ON" : "OFF"}
                </button>
              ...
              <button
                style={{
                  ...S.btnStart,
                  ...(poolSize < 4 ? S.btnDisabled : {}),
                }}
                onClick={startQuiz}
                disabled={poolSize < 4}
              >
                {poolSize < 4
                  ? `단어 최소 4개 필요 (현재 ${poolSize}개)`
                  : `🚀 퀴즈 시작! (${
                      quizCount === 9999
                        ? poolSize
                        : Math.min(quizCount, poolSize)
          ...
          <div key={animKey}>
            ...
            <div style={S.progressLabel}>
              ...
              <span style={S.scoreLabel}>
                점수: {score} / {current + (confirmed ? 1 : 0)}
              </span>
            </div>
            <div style={S.questionCard}>
              <div style={S.questionTag}>
              ...
              {(() => {
                ...
                const st = w ? STATUS_CONFIG[w.status] : null;
                return st ? (
                  <div
                    style={{
                      ...S.statusPill,
            ...
            <div className="choices-grid" style={S.choicesGrid}>
              {q.choices.map((choice, idx) => {
                let extra = {};
                if (confirmed) {
                  if (choice === q.answer) extra = S.choiceCorrect;
                  else if (choice === selected) extra = S.choiceWrong;
                } else if (choice === selected) extra = S.choiceSelected;
                return (
                  <button
                    key={idx}
                    style={{ ...S.choiceBtn, ...extra }}
        ...
        {mode === MODES.RESULT && (
          <div style={S.resultWrap}>
            ...
            <div style={S.resultStats}>
              {[
                ["mastered", "✅ 암기완료", "#60c880"],
                ["hard", "🔥 어려움", "#f07050"],
                ["learning", "📖 학습중", "#9090b0"],
              ].map(([st, label, color]) => (
                <div key={st} style={{ ...S.resultStatItem, color }}>
                  <span style={S.resultStatNum}>
                    {words.filter((w) => w.status === st).length}
                  </span>
                ...
                {wrongWords.map((q, i) => {
                  ...
                  return w ? (
                    <div key={i} style={S.wrongItem}>
                      ...
                      <SpeakBtn text={w.hebrew} onSpeak={speak} />
                      <span style={{ color: "#a0a0b0", margin: "0 4px" }}>
                        →
                      </span>
            ...
            <div style={S.saveReminder}>
              💾 퀴즈 결과를 저장해두세요!
              <button style={S.btnExportSmall} onClick={exportWords}>
                지금 저장
              </button>
            </div>
            <div className="result-btn-row" style={S.resultBtnRow}>
              <button style={{ ...S.btnStart, flex: 1 }} onClick={startQuiz}>
                🔄 다시 풀기
              </button>
              <button
                style={{ ...S.btnQuit, flex: 1 }}
...
const S = {
  ...
  bgDeco1: {
    ...
    borderRadius: "50%",
    background:
      "radial-gradient(circle,rgba(196,160,80,0.12) 0%,transparent 70%)",
    pointerEvents: "none",
  },
  bgDeco2: {
    position: "fixed",
    ...
    borderRadius: "50%",
    background:
      "radial-gradient(circle,rgba(100,80,180,0.15) 0%,transparent 70%)",
    pointerEvents: "none",
  },
  container: {
    maxWidth: "700px",
  ...
  toast: {
    ...
    background: "rgba(60,180,100,0.97)",
    color: "#fff",
    padding: "12px 22px",
    borderRadius: "12px",
    fontSize: "0.88rem",
  ...
  btnMerge: {
    ...
    padding: "12px 10px",
    borderRadius: "10px",
    background: "linear-gradient(135deg,#c4a050,#e8c875)",
    border: "none",
    color: "#1a1820",
    fontWeight: 700,
    cursor: "pointer",
  ...
  progressFill: {
    height: "100%",
    background: "linear-gradient(90deg,#c4a050,#e8c875)",
    borderRadius: "3px",
  ...
  choiceSelected: {
    ...
    borderColor: "rgba(100,80,200,0.6)",
    color: "#d0c8ff",
  },
  choiceCorrect: {
    background: "rgba(60,180,100,0.2)",
    borderColor: "rgba(60,180,100,0.6)",
    color: "#80e8a0",
  },
  choiceWrong: {
  ...
  btnNext: {
    ...
    padding: "15px",
    borderRadius: "12px",
    background: "linear-gradient(135deg,#6040c8,#9060f0)",
    border: "none",
    color: "#fff",
    fontWeight: 700,
    cursor: "pointer",
  ...
  resultCircle: {
    ...
    border: "3px solid rgba(196,160,80,0.4)",
    display: "flex",
    alignItems: "center",
    justifyContent: "center",
    margin: "0 auto 20px",
