...
function Icon({name,size=18,color="currentColor",style={}}){
  ...
  switch(name){
    ...
    case"variant":return<svg {...p}><polyline points="16 3 21 3 21 8"/><line x1="4" y1="20" x2="21" y2="3"/><polyline points="21 16 21 21 16 21"/><line x1="15" y1="15" x2="21" y2="21"/></svg>;
...
export default function HebrewQuiz() {
  ...
  useEffect(()=>{
    ...
    friends.forEach(f=>{
      ...
      dmUnsubMapRef.current[f.uid]=onSnapshot(q2,snap=>{
        const msgs=snap.docs.map(d=>({id:d.id,...d.data()}));
        // DM 탭에서 이 친구와 대화 중이면 읽음 처리
        if(activeTabRef.current==="chat"&&dmTarget?.uid===f.uid){
          setDmUnreadMap(m=>({...m,[f.uid]:0}));
        } else {
          const unread=msgs.filter(m=>m.uid!==user.uid).length;
  ...
  const sendGlobalMessage=async()=>{
    ...
    if(editingMsgId){
      ...
      }catch(e){console.error("Edit failed:",e);showToast("수정 실패: "+e.message,"err");}
    } else {
      setChatInput("");
      try{
      ...
      }catch(e){console.error("Send failed:",e);showToast("전송 실패: "+e.message,"err");}
    }
  };
  const deleteGlobalMessage=async(msgId)=>{
    ...
    }catch(e){console.error("Delete failed:",e);showToast("삭제 실패: "+e.message,"err");}
  };
