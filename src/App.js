...
const UI_TEXT = {
  ko: {
    ...
    reversoImport:"🔍 Reverso 동사 변형 가져오기", rootSearch:"🌿 어근으로 단어 검색 (Pealim)",
    variantExcel:"📥 변형 엑셀 불러오기", verbForm:"📋 동사변형 양식 불러오기", formDownload:"⬇️ 동사변형 양식 다운로드",
    refreshVariants:"🔄 기존 단어 변형 다시 불러오기",
    cardStyle:"카드 스타일:", menuStyle:"🪄 메뉴", inlineStyle:"📋 인라인",
    defaultWordbook:"기본 단어장 (전체)", addToWordbook:"추가할 단어장 선택",
    sortDefault:"기본순", sortHebAsc:"히브리어 ↑", sortHebDesc:"히브리어 ↓",
    sortMeanAsc:"뜻 ↑", sortMeanDesc:"뜻 ↓", sortHardFirst:"🔥 어려운 것 먼저",
    sortMasteredFirst:"✅ 암기 먼저", sortWrongFirst:"❌ 오답 많은 것 먼저",
    rootGroupView:"🌿 어근별로 단어 묶어보기",
    learningOnly:(n)=>`📖 학습중 (${n})`,
    variantQuizTitle:"🔀 변형 퀴즈", variantUnavailable:"변형 없음",
    variantTypeSelect:"변형 유형 선택", wordRangeNote:"(변형 있는 단어만 표시)",
  ...
  en: {
    ...
    variantExcel:"📥 Variant Excel", verbForm:"📋 Verb Form", formDownload:"⬇️ Download Form",
    refreshVariants:"🔄 Refresh Variants",
    cardStyle:"Card style:", menuStyle:"🪄 Menu", inlineStyle:"📋 Inline",
    defaultWordbook:"Default (All)", addToWordbook:"Add to wordbook",
...
const STATUS_LABELS = {
  ko: { learning:"학습중", mastered:"암기완료", hard:"어려움" },
  en: { learning:"Learning", mastered:"Mastered", hard:"Hard" },
};
const STATUS_CONFIG = {
...
function SpeakOnceBtn({text,onSpeak,muted=false,repeatN=1}){
  ...
  return(
    <button onClick={handle}
      style={{background:playing?"rgba(200,60,60,0.25)":"rgba(196,160,80,0.1)",
        border:playing?"2px solid rgba(200,60,60,0.6)":"1px solid rgba(196,160,80,0.3)",
        borderRadius:"7px",cursor:muted?"default":"pointer",padding:"5px 10px",
        fontSize:playing?"0.85rem":"0.9rem",lineHeight:1,opacity:muted?0.3:1,
        fontWeight:playing?700:400,transition:"all 0.1s"}}>
      {playing?`⏹ ${count}`:"🔈"}
    </button>
  );
}
...
export default function HebrewQuiz() {
          ...
          if(snap.exists()){
            ...
            if(cloud&&cloud.length){
              if(hasLocal && !alreadySynced){
                ...
                setWordsRaw(cloud); saveWords(cloud);
                if(!alreadySynced) showToast(uiLang==="en"?"☁️ Cloud wordbook loaded!":"☁️ 클라우드 단어장을 불러왔어요!");
                localStorage.setItem(syncKey,"1");
              }
            } else {
              ...
              if(hasLocal){
                ...
                localStorage.setItem(syncKey,"1");
                showToast(uiLang==="en"?"☁️ Saved to cloud!":"☁️ 기존 단어장을 클라우드에 저장했어요!");
              }
            }
          } else {
  ...
  const handleMerge=(choice)=>{
    ...
    if(choice==="cloud"){
      ...
      showToast(uiLang==="en"?"☁️ Replaced with cloud data!":"☁️ 클라우드 단어장으로 교체했어요!");
    } else if(choice==="local"){
      // 로컬 유지, 클라우드에 로컬 업로드
      const local=loadWords();
      if(user) setDoc(doc(fbDb,"users",user.uid),{words:local,updatedAt:new Date().toISOString()});
      showToast(uiLang==="en"?"💾 Saved to cloud!":"💾 기존 단어장을 클라우드에 저장했어요!");
    } else {
      // 병합: 히브리어 기준 중복 제거
      const local=loadWords();
      ...
      showToast(uiLang==="en"?`☁️ Merged! ${merged.length} words total`:`☁️ 병합 완료! 총 ${merged.length}개 단어`);
    }
  ...
  const signInGoogle = async()=>{
    try{ await signInWithPopup(fbAuth, new GoogleAuthProvider()); showToast(uiLang==="en"?"Signed in! Loading...":"로그인 성공! 단어장을 불러오는 중..."); }
  ...
  const [bulkWalletOpen,setBulkWalletOpen]=useState(false); // 단어장 선택 팝업용 wordId
  ...
  const applyVariantPaste=(text)=>{
    ...
    showToast(uiLang==="en"?`📋 ${Math.min(lines.length,VARIANT_PASTE_ORDER.length)} variants pasted`:`📋 ${Math.min(lines.length,VARIANT_PASTE_ORDER.length)}개 변형을 입력했어요!`);
  };
  ...
  const saveVariantDraft=(wordId)=>{
    ...
    if(diff<0) showToast(uiLang==="en"?`✅ Saved ${variants.length} (${Math.abs(diff)} removed)`:`✅ ${variants.length}개 저장! (${Math.abs(diff)}개 삭제됨)`);
    else showToast(uiLang==="en"?`✅ ${variants.length} variants saved!`:`✅ 변형 ${variants.length}개 저장됐어요!`);
  };
  ...
  const searchWordByMeaning=async()=>{
    ...
    try{
      ...
      if(currentBook==="hebrew"){
        ...
        if(!translated){setWordSearchError(uiLang==="en"?"No translation result.":"번역 결과가 없어요.");return;}
    ...
    }catch(e){setWordSearchError((uiLang==="en"?"Error: ":"오류: ")+e.message);}
  ...
  const addSelectedWordSearchResults=()=>{
    ...
    showToast(uiLang==="en"?`✅ Added ${newWords.length} words!`:`✅ ${newWords.length}개 단어를 추가했어요!`);
  ...
  const searchByRoot=async()=>{
    ...
    }catch(e){setRootSearchError((uiLang==="en"?"Error: ":"오류: ")+e.message);}
    finally{setRootSearchLoading(false);}
  };

  const addSelectedRootWords=()=>{
    if(!rootSelected.size){setRootSearchError(uiLang==="en"?"Please select words":"단어를 선택해주세요");return;}
  ...
  const searchPealim=async()=>{
    if(!pealimRoot.trim()){setPealimError(uiLang==="en"?"Please enter a verb":"동사를 입력해주세요");return;}
    ...
    }catch(e){setPealimError((uiLang==="en"?"Load error: ":"불러오는 중 오류: ")+e.message);}
    finally{setPealimLoading(false);}
  };

  // 선택된 결과들 일괄 단어장 추가
  const addSelectedPealimWords=async()=>{
    if(!pealimSelected.size){ setPealimError(uiLang==="en"?"Please select a word first":"단어를 먼저 선택해주세요"); return; }
    setPealimLoading(true); setPealimError("");

    const selectedList = [...pealimSelected].map(idx=>pealimResults[idx]).filter(Boolean);
    if(!selectedList.length){ setPealimLoading(false); setPealimError(uiLang==="en"?"Nothing selected":"선택된 항목이 없어요"); return; }
    ...
    for(let i=0; i<selectedList.length; i++){
      ...
      try{
        ...
        if(!res.ok) throw new Error((uiLang==="en"?"Server error ":"서버 오류 ")+res.status);
        const data = await res.json();
    ...
    if(newWords.length===0){
      ...
      setPealimError(uiLang==="en"?"Failed to add word. Please try again.":"단어 추가에 실패했어요. 다시 시도해주세요.");
      return;
    }
    ...
    showToast(uiLang==="en"?`✅ Added ${newWords.length} words (${withVariants} with variants)`:`✅ ${newWords.length}개 단어 추가! (변형 ${withVariants}개 포함)`);
  ...
  const fetchPealimConjugation=async(url, root)=>{
    ...
    }catch(e){setPealimError((uiLang==="en"?"Variant fetch error: ":"변형 데이터를 가져오는 중 오류: ")+e.message);}
    finally{setPealimLoading(false);}
  };
  ...
  const importPealimToWord=(wordId)=>{
    ...
    showToast(uiLang==="en"?`✅ ${variants.length} variants saved!`:`✅ ${variants.length}개 변형을 단어장에 저장됐어요!`);
  };
  ...
  const downloadTemplate=()=>{
    ...
    a.href=url; a.download="Hebrew_Conjugation_Template.xlsx"; a.click();
  ...
  const refreshAllVariants=async()=>{
    ...
    showToast(uiLang==="en"?`✅ ${ok} ok / ${log.length-ok} failed`:`✅ ${ok}개 성공 / ${log.length-ok}개 실패`);
  };
  ...
  const addNewWordFromPealim=()=>{
    ...
    if(exists){
      ...
      showToast(uiLang==="en"?`✅ Updated ${variants.length} variants for "${pealimPreview.infinitive}"`:`✅ "${pealimPreview.infinitive}" 변형 ${variants.length}개 업데이트됐어요!`);
    } else {
      ...
      showToast(uiLang==="en"?`✅ Added "${pealimPreview.infinitive}" with ${variants.length} variants`:`✅ "${pealimPreview.infinitive}" 단어와 변형 ${variants.length}개가 추가됐어요!`);
    }
    setShowPealimModal(false); setPealimPreview(null); setPealimRoot(""); setPealimResults([]);
  };
  ...
  const handleVariantExcel=async(e)=>{
    ...
    try{
      ...
      if(!entries.length){showToast(uiLang==="en"?"No variant data found.":"변형 데이터를 찾을 수 없어요.","err");return;}
      ...
      showToast(uiLang==="en"?`📥 Added ${added} variants to ${updated} words`:`📥 ${updated}개 단어에 변형 ${added}개 추가됐어요!`);
    }catch(err){showToast((uiLang==="en"?"Cannot read file: ":"파일을 읽을 수 없어요: ")+err.message,"err");}
    e.target.value="";
  };
  ...
  const startVariantQuiz=()=>{
    ...
    if(!pool.length){showToast(uiLang==="en"?"No words for selected variant types.":"선택한 변형 유형의 단어가 없어요.","err");return;}
  ...
  const copyToClipboard=async()=>{ const text=JSON.stringify({version:1,exportedAt:new Date().toISOString(),words},null,2); try{await navigator.clipboard.writeText(text); showToast(uiLang==="en"?"📋 Copied to clipboard!":"📋 클립보드에 복사됐어요!");}catch{const ta=document.createElement("textarea"); ta.value=text; document.body.appendChild(ta); ta.select(); document.execCommand("copy"); document.body.removeChild(ta); showToast(uiLang==="en"?"📋 Copied to clipboard!":"📋 클립보드에 복사됐어요!");} };
  const importFromText=()=>{ try{ const parsed=JSON.parse(pasteText); const raw=Array.isArray(parsed)?parsed:(parsed.words||[]); const imported=raw.map(w=>({id:Date.now()+Math.random(),hebrew:(w.hebrew||"").trim(),meaning:(w.meaning||"").trim(),status:["learning","mastered","hard"].includes(w.status)?w.status:"learning",streak:w.streak||0,wrongCount:w.wrongCount||0})).filter(w=>w.hebrew&&w.meaning); if(!imported.length){showToast(uiLang==="en"?"No words to import.":"불러올 단어가 없어요.","err");return;} setImportPreview({words:imported,fileName:uiLang==="en"?"From clipboard":"클립보드에서 붙여넣기"}); setShowPasteModal(false); setPasteText(""); }catch{showToast("올바른 형식이 아니에요.","err");} };
  const importFromBatchText=()=>{ const raw=batchTextRef.current?batchTextRef.current.value:""; const parsed=parseTextFormat(raw); if(!parsed.length){showToast(uiLang==="en"?"No words recognized. Use שלום=peace format.":"인식된 단어가 없어요. שלום=평화 형식으로 입력해주세요.","err");return;} setImportPreview({words:parsed.map(w=>({...w,id:Date.now()+Math.random(),status:"learning",streak:0,wrongCount:0})),fileName:`텍스트 형식 (${parsed.length}개)`}); setShowBatchModal(false); if(batchTextRef.current) batchTextRef.current.value=""; };
  const handleFileChange=(e)=>{ const file=e.target.files[0]; if(!file) return; const reader=new FileReader(); reader.onload=(ev)=>{ try{ const parsed=JSON.parse(ev.target.result); const raw=Array.isArray(parsed)?parsed:(parsed.words||[]); const imported=raw.map(w=>({id:Date.now()+Math.random(),hebrew:(w.hebrew||"").trim(),meaning:(w.meaning||"").trim(),status:["learning","mastered","hard"].includes(w.status)?w.status:"learning",streak:w.streak||0,wrongCount:w.wrongCount||0})).filter(w=>w.hebrew&&w.meaning); if(!imported.length){showToast(uiLang==="en"?"No words to import.":"불러올 단어가 없어요.","err");return;} setImportPreview({words:imported,fileName:file.name}); }catch{showToast("파일을 읽을 수 없어요.","err");} }; reader.readAsText(file); e.target.value=""; };
  // ── 히브리어 동사변형 엑셀 양식 파싱 ──
  ...
  const handleVerbFormExcel=(e)=>{
    ...
    reader.onload=(ev)=>{
      try{
        ...
        if(variantCount===0){showToast(uiLang==="en"?"No variant data found. Check Excel format.":"변형 데이터가 없어요. 엑셀 양식을 확인해주세요.","err");return;}
        ...
        showToast(uiLang==="en"?`✅ Loaded ${variantCount} variants. Add meaning and save.`:`✅ ${variantCount}개 변형 불러옴! 뜻을 입력하고 추가해주세요.`);
      }catch(err){showToast((uiLang==="en"?"Cannot read file: ":"파일을 읽을 수 없어요: ")+err.message,"err");}
    };
  ...
  const handleCSVChange=async(e)=>{
    ...
    if (isXlsx) {
      ...
      try {
        ...
        if (!parsed.length) { showToast(uiLang==="en"?"No words recognized. Check column A: Hebrew, B: Meaning.":"인식된 단어가 없어요. A열: 히브리어, B열: 뜻 형식인지 확인해주세요.", "err"); return; }
        setImportPreview({ words: parsed.map(w=>({...w,id:Date.now()+Math.random(),status:"learning",streak:0,wrongCount:0})), fileName:`${file.name}` });
      } catch { showToast(uiLang==="en"?"Cannot read Excel file.":"엑셀 파일을 읽을 수 없어요.", "err"); }
    } else {
      // CSV / TSV / TXT 파일
      const reader = new FileReader();
      reader.onload = (ev) => {
        const parsed = parseCSV(ev.target.result);
        if (!parsed.length) { showToast(uiLang==="en"?"No words recognized. Check col 1: Hebrew, col 2: Meaning.":"인식된 단어가 없어요. 첫째 열: 히브리어, 둘째 열: 뜻 형식인지 확인해주세요.", "err"); return; }
        setImportPreview({ words: parsed.map(w=>({...w,id:Date.now()+Math.random(),status:"learning",streak:0,wrongCount:0})), fileName:`${file.name}` });
      };
  ...
  const startRootQuiz=(root, type)=>{
    ...
    if(type==="mcq"){
      if(rootWords.length<2){showToast(uiLang==="en"?"MCQ needs at least 2 words.":"객관식은 단어 2개 이상 필요해요","err");return;}
      ...
      if(!pairs.length){showToast(uiLang==="en"?"No data for selected variant types.":"선택된 변형 유형의 데이터가 없어요","err");return;}
  ...
  const deleteWord=(id)=>{
    if(walletFilter){
      // 커스텀 단어장 탭에서 삭제 → 해당 단어장에서만 제거, 단어 자체는 유지
      saveWallets(wallets.map(wl=>wl.id===walletFilter
        ?{...wl,wordIds:wl.wordIds.filter(i=>i!==id)}
        :wl));
    } else {
      // 전체/학습중/어려움/완료 탭에서 삭제 → 단어 완전 삭제 + 모든 단어장에서 제거
...
`}</style>
      ...
      <Modal show={showPasteModal} onClose={()=>setShowPasteModal(false)} title={uiLang==="en"?"📋 Import from Text":"📋 텍스트로 불러오기"}>
        <p style={S.modalSub}>{uiLang==="en"?"Paste JSON text copied from the app":"📋 복사로 저장한 JSON 텍스트를 붙여넣어주세요"}</p>
        ...
        <div className="modal-btn-row" style={S.modalBtnRow}>
          <button style={S.btnMerge} onClick={importFromText}>{uiLang==="en"?"✅ Import":"✅ 불러오기"}</button>
      ...
      {showWalletModal&&(
        <div style={S.modalOverlay} onClick={()=>setShowWalletModal(false)}>
          <div style={{...S.modal,maxWidth:"480px",maxHeight:"80vh",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
            <h3 style={S.modalTitle}>{uiLang==="en"?"📚 Wordbooks":"📚 커스텀 단어장"}</h3>
                  ...
                  {wlWords.length>0?(
                    <>
                      ...
                      <div style={{display:"flex",gap:"6px"}}>
                        <button onClick={()=>{
                          if(wlWords.length<4){showToast(uiLang==="en"?"MCQ needs at least 4 words.":"객관식은 4개 이상 필요해요","err");return;}
        ...
        <div style={S.modalOverlay} onClick={()=>setShowWordSearchModal(false)}>
          <div style={{...S.modal,maxWidth:"480px"}} onClick={e=>e.stopPropagation()}>
            <h3 style={S.modalTitle}>🔎 {
              uiLang==="en"
                ?(currentBook==="hebrew"?"Search Hebrew by meaning":currentBook==="english"?"Find English words":"Find Korean words")
                :(currentBook==="hebrew"?"뜻으로 히브리어 검색":currentBook==="english"?"한국어/히브리어로 영어 찾기":"영어/히브리어로 한국어 찾기")
            }</h3>
            <p style={S.modalSub}>{
              currentBook==="hebrew"?(uiLang==="en"?"Enter Korean or English to find Hebrew words. Korean is auto-translated.":"한국어 또는 영어로 입력하면 히브리어 단어를 찾아줘요. 한국어는 자동 번역돼요. 예: 사랑, love"):
              currentBook==="english"?(uiLang==="en"?"Enter Korean or Hebrew to find English words. e.g. apple, שלום":"한국어 또는 히브리어를 입력하면 영어 단어를 찾아줘요. 예: 사과, שלום"):
              (uiLang==="en"?"Enter English or Hebrew to find Korean meaning. e.g. love, שלום":"영어 또는 히브리어를 입력하면 한국어 뜻을 찾아줘요. 예: love, שלום")
            }</p>
            <div style={{display:"flex",gap:"8px",marginBottom:"10px"}}>
              <input style={{...S.input,flex:1}}
                placeholder={currentBook==="hebrew"?"사랑, love...":currentBook==="english"?(uiLang==="en"?"Korean input (e.g. apple)":"한국어 입력 (예: 사과)"):(uiLang==="en"?"English or Hebrew":"영어 또는 히브리어 입력")}
              ...
              <button onClick={searchWordByMeaning} disabled={wordSearchLoading}
                ...
                {wordSearchLoading?"...":(uiLang==="en"?"Search":"검색")}
              </button>
            </div>
              ...
              <div>
                ...
                <div style={{maxHeight:"340px",overflowY:"auto",display:"flex",flexDirection:"column",gap:"5px"}}>
                  {wordSearchResults.map((r,i)=>{
                    ...
                    const posLabels={verb:uiLang==="en"?"Verb":"동사",noun:uiLang==="en"?"Noun":"명사",adj:uiLang==="en"?"Adj":"형용사"};
                    return(
          ...
          <div style={{...S.modal,maxWidth:"480px"}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:"flex",gap:"8px",marginBottom:"10px"}}>
              ...
              <button onClick={searchByRoot} disabled={rootSearchLoading}
                ...
                {rootSearchLoading?"...":(uiLang==="en"?"Search":"검색")}
              </button>
            </div>
      ...
      {showPealimModal&&(
        <div style={S.modalOverlay}>
          <div style={{...S.modal,maxWidth:"500px",maxHeight:"88vh",overflowY:"auto"}}>
            <h3 style={S.modalTitle}>{uiLang==="en"?"🔍 Reverso Conjugation Import":"🔍 Reverso 동사 변형 불러오기"}</h3>
            ...
            <div style={{display:"flex",gap:"8px",marginBottom:"12px"}}>
              ...
              <button
                ...
                {pealimLoading?(uiLang==="en"?"Searching...":"검색 중..."):(uiLang==="en"?"Search":"검색")}
              </button>
            </div>
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px",flexWrap:"wrap",gap:"6px"}}>
                  ...
                  {pealimSelected.size>0&&(
                    <button onClick={addSelectedPealimWords} disabled={pealimLoading}
                      ...
                      {pealimLoading?(uiLang==="en"?"Loading...":"불러오는 중..."):(uiLang==="en"?` ✅ Add ${pealimSelected.size}`:`✅ ${pealimSelected.size}개 단어장에 추가`)}
                    </button>
                  )}
                </div>
                {pealimSelected.size>0&&(
                  <div style={{fontSize:"0.72rem",color:"#50c898",marginBottom:"8px",padding:"6px 10px",background:"rgba(80,160,120,0.08)",borderRadius:"6px"}}>
                    {uiLang==="en"?"Auto-imports meaning and conjugation (may take a moment).":"뜻과 변형을 자동으로 불러와서 저장해요 (시간이 조금 걸릴 수 있어요)"}
                  </div>
                )}
                {/* 결과 목록 */}
                    ...
                    return(
                      <div key={i} style={{display:"flex",gap:"8px",alignItems:"center",padding:"8px 12px",borderRadius:"10px",
                        ...
                        {!alreadyAdded&&<button onClick={e=>{e.stopPropagation();fetchPealimConjugation(r.url,pealimRoot);}}
                          ...
                          title={uiLang==="en"?"Variant preview":"변형 미리보기"}>
                          미리보기
                        </button>}
                      </div>
                    );
                ...
                <div style={{background:"rgba(80,160,120,0.08)",border:"1px solid rgba(80,160,120,0.2)",borderRadius:"10px",padding:"14px",marginBottom:"12px"}}>
                  <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"10px",flexWrap:"wrap"}}>
                    ...
                    <span style={{fontSize:"0.72rem",background:"rgba(80,160,120,0.2)",padding:"3px 10px",borderRadius:"6px",color:"#50c898",fontWeight:600}}>
                      {Object.keys(pealimPreview.variants||{}).length}{uiLang==="en"?" variants":" 개 변형"}
                    </span>
                  </div>
                  {/* 뜻 — 항상 수정 가능 */}
      ...
      <Modal show={showBatchModal} onClose={()=>setShowBatchModal(false)} title={uiLang==="en"?"📝 Add Words by Text":"📝 텍스트 형식으로 단어 추가"}>
        <p style={S.modalSub}>{uiLang==="en"?"One word per line in the format: Hebrew=Meaning":"한 줄에 하나씩 히브리어=뜻 형식으로 입력하세요"}</p>
        ...
        return(
          <div style={{...S.modalOverlay,alignItems:"flex-start",paddingTop:"20px",overflowY:"auto"}}>
            <div style={{...S.modal,maxWidth:"600px",maxHeight:"90vh",overflowY:"auto"}}>
                      ...
                      return(
                        <div key={tid} style={{display:"flex",flexDirection:"column",gap:"3px"}}>
                          ...
                          <input
                            ...
                            placeholder={uiLang==="en"?"Enter Hebrew...":"히브리어 입력..."}
                            lang="he" spellCheck={false} autoCorrect="off"
              ...
              {/* 저장/취소 버튼 */}
              <div style={{display:"flex",gap:"8px",marginTop:"8px",position:"sticky",bottom:0,background:"#1E1D24",paddingTop:"12px"}}>
          ...
          <div style={S.modal}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:"8px",marginBottom:"4px"}}>
              ...
              <button style={{...S.btnReplace,padding:"12px"}} onClick={()=>handleMerge("cloud")}>
                {uiLang==="en"?`☁️ Use Cloud (${pendingCloudWords.length} words)`:`☁️ 클라우드 사용 — 클라우드 단어장으로 교체 (${pendingCloudWords.length}개)`}
              </button>
              <button style={{...S.btnCancel2,padding:"12px"}} onClick={()=>handleMerge("local")}>
                {uiLang==="en"?"💾 Keep Local — Save local words to cloud":"💾 기기 단어 유지 — 현재 기기 단어장을 클라우드에 저장"}
              </button>
            </div>
          </div>
        </div>
      ...
      {importPreview&&(
        <div style={S.modalOverlay}><div style={S.modal}>
          <h3 style={S.modalTitle}>{uiLang==="en"?"📥 Import Words":"📥 단어 불러오기"}</h3>
          <p style={S.modalSub}>{uiLang==="en"?"Source:":"출처:"} <span style={{color:"#c4a050"}}>{importPreview.fileName}</span></p>
          <p style={S.modalSub}><b style={{color:"#e8e6f0"}}>{importPreview.words.length}</b> {uiLang==="en"?"words found":"개 단어 발견"}</p>
          ...
          <div className="modal-btn-row" style={S.modalBtnRow}>
            ...
            <button style={S.btnReplace} onClick={()=>confirmImport(false)}>{uiLang==="en"?"🔄 Replace All":"🔄 전체 교체"}</button>
        ...
        {mode===MODES.LIST&&(
          <>
            <button
              ...
              style={S.floatBtn}
              title={uiLang==="en"?"Top":"맨 위로"}>
              ↑
            </button>
            <button
              onClick={()=>window.scrollTo({top:document.body.scrollHeight,behavior:"smooth"})}
              style={{...S.floatBtn,bottom:"70px"}}
              title={uiLang==="en"?"Bottom":"맨 아래로"}>
              ↓
            </button>
          </>
        )}
        ...
        {mode===MODES.LIST&&(
          <div>
            ...
            {currentBook==="hebrew"&&<div style={{...S.ioCard,borderColor:"rgba(80,160,120,0.2)"}}>
              <SectionHeader sectionKey="import" title={T.importWords} color="#50c898"/>
              {openSections.import&&<div style={{marginTop:"10px"}}>
                <div style={{display:"flex",gap:"8px",flexWrap:"wrap",marginBottom:"10px"}}>
                  ...
                  <button onClick={refreshAllVariants} disabled={refreshingVariants}
                    ...
                    {refreshingVariants?"🔄 업데이트 중...":T.refreshVariants}
                  </button>
                </div>
                {/* 변형 불러오기 결과 로그 */}
                {refreshLog.length>0&&<div style={{marginBottom:"8px"}}>
                  <button onClick={()=>setShowRefreshLog(v=>!v)}
                    ...
                    {showRefreshLog?(uiLang==="en"?"▲ Hide":"▲ 결과 숨기기"):(uiLang==="en"?"▼ Results":"▼ 불러오기 결과")} ({refreshLog.filter(l=>l.status==="ok").length}개 성공 / {refreshLog.filter(l=>l.status==="fail").length}개 실패)
                  </button>
            ...
            <div style={{display:"flex",gap:"8px",marginBottom:"10px",flexWrap:"wrap",alignItems:"center"}}>
              ...
              <select value={sortBy} onChange={e=>{setSortBySave(e.target.value);setPage(0);}}
                ...
                <option value="hebrew_asc">{T.sortHebAsc}</option>
                <option value="hebrew_desc">{T.sortHebDesc}</option>
                <option value="meaning_asc">{T.sortMeanAsc}</option>
                <option value="meaning_desc">{T.sortMeanDesc}</option>
              ...
              <div style={{display:"flex",gap:"4px"}}>
                {[10,20,9999].map(n=>(
                  <button key={n} style={{...S.optBtn,padding:"8px 10px",fontSize:"0.78rem",...(pageSize===n?S.optBtnActive:{})}}
                    ...
                    {n===9999?(uiLang==="en"?"All":"전체"):(uiLang==="en"?n:n+"개")}
                  </button>
                ))}
              </div>
            </div>
            ...
            {currentBook==="hebrew"&&words.some(w=>w.root)&&(
              <button onClick={()=>setRootGroupView(v=>!v)}
                ...
                {rootGroupView?(uiLang==="en"?"🌿 Hide Root Groups":T.rootGroupView+" 끄기"):(uiLang==="en"?"🌿 Group by Root":T.rootGroupView)}
              </button>
            )}
                      ...
                      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 14px",background:"rgba(80,160,120,0.08)",flexWrap:"wrap",gap:"8px"}}>
                        ...
                        <div style={{display:"flex",gap:"6px",flexWrap:"wrap"}}>
                          <button onClick={async()=>{
                            ...
                            setRefreshingVariants(false);
                            showToast(uiLang==="en"?`✅ ${root} root variants updated!`:`✅ ${root} 어근 변형 업데이트 완료!`);
              ...
              <div style={{display:"flex",alignItems:"center",gap:"8px",padding:"8px 12px",borderRadius:"8px",
                ...
                <button onClick={()=>setShowWalletModal(true)}
                  style={{padding:"4px 10px",borderRadius:"7px",background:"rgba(196,160,80,0.2)",
                    border:"1px solid rgba(196,160,80,0.4)",color:"#c4a050",cursor:"pointer",fontSize:"0.75rem",fontWeight:600,flexShrink:0}}>
                  + 만들기
                </button>
              </div>
            )}
            ...
            {walletFilter&&(()=>{
              const wl=wallets.find(x=>x.id===walletFilter);
              return wl?(
                <div style={{padding:"6px 12px",borderRadius:"8px",marginBottom:"6px",
                  background:`${wl.color}12`,border:`1px solid ${wl.color}30`,
                  fontSize:"0.75rem",color:wl.color,display:"flex",alignItems:"center",gap:"6px"}}>
                  <span style={{width:"8px",height:"8px",borderRadius:"50%",background:wl.color,flexShrink:0,display:"inline-block"}}/>
                  <span><b>{wl.name}</b> {uiLang==="en"?"Viewing wordbook — Delete removes from this list only":"단어장 보기 중 — 삭제 시 이 단어장에서만 제거돼요 (단어는 유지)"}</span>
                </div>
              ):null;
            })()}
            {/* 필터 탭 */}
                            ...
                            return(<>
                              ...
                              {sel.size>0&&(
                                <button onClick={()=>{
                                  ...
                                  showToast(`✅ ${selectedIds.size}${uiLang==="en"?" words":" 개"} → ${[...sel].map(id=>wallets.find(w=>w.id===id)?.name).join(", ")} 추가!`);
                    ...
                    {cardStyle==="inline"&&(
                      <div style={{display:"flex",alignItems:"center",gap:"4px",marginTop:"4px",flexWrap:"wrap"}}>
                        ...
                        {(()=>{ const sc=STATUS_CONFIG[w.status]; const order=["learning","hard","mastered"]; const next=order[(order.indexOf(w.status)+1)%3];
                          return <button onClick={()=>setManualStatus(w.id,next)} style={{padding:"2px 6px",borderRadius:"5px",border:`1px solid ${sc.border}`,background:sc.bg,color:sc.color,cursor:"pointer",fontSize:"0.65rem",fontWeight:600}}>{sc.emoji} {(uiLang==="en"?sc.labelEn:sc.labelKo)}</button>;
                        })()}
                        ...
                        <button onClick={()=>{
  const msg=walletFilter?(uiLang==="en"?"Remove from wordbook":"이 단어장에서만 제거"):(uiLang==="en"?"Delete word":"단어 삭제");
  if(window.confirm(msg+(uiLang==="en"?"?":"할까요?"))) deleteWord(w.id);
}} style={{...S.btnEdit,color:"#f07070",fontSize:"0.72rem",opacity:0.6}}>{walletFilter?(uiLang==="en"?"Remove":"제거"):"🗑️"}</button>
                      </div>
                    )}
                  </div>
                  {/* 오른쪽: 🔈 + ··· 메뉴 (menu 스타일만) */}
                          ...
                          <div onClick={e=>e.stopPropagation()}
                            ...
                            <div style={{display:"flex",gap:"4px",borderTop:"1px solid rgba(255,255,255,0.07)",paddingTop:"5px"}}>
                              ...
                              {wallets.length>0&&<button onClick={e=>{e.stopPropagation();setWalletPickWord(w.id);}}
                                ...
                                📚 {wallets.filter(wl=>wl.wordIds.includes(w.id)).length>0?"✓":""}
                              </button>}
                              <button onClick={()=>{
                                const msg=walletFilter?(uiLang==="en"?"Remove from this wordbook? (word is kept)":"이 단어장에서만 제거할까요? (단어 자체는 유지됩니다)"):(uiLang==="en"?"Permanently delete this word?":"단어를 완전히 삭제할까요?");
                                ...
                                {walletFilter?(uiLang==="en"?"Remove":"제거"):"🗑️"}
                              </button>
                            </div>
                          </div>
                        )}
            ...
            {pageSize!==9999&&totalPages>1&&(
              <div style={{display:"flex",justifyContent:"center",gap:"6px",marginBottom:"14px",flexWrap:"wrap"}}>
                <button style={{...S.scrollBtn,...(page===0?{opacity:0.3,cursor:"not-allowed"}:{})}} onClick={()=>page>0&&setPage(p=>p-1)} disabled={page===0}>{uiLang==="en"?"← Prev":"← 이전"}</button>
                ...
                <button style={{...S.scrollBtn,...(page===totalPages-1?{opacity:0.3,cursor:"not-allowed"}:{})}} onClick={()=>page<totalPages-1&&setPage(p=>p+1)} disabled={page===totalPages-1}>{uiLang==="en"?"Next →":"다음 →"}</button>
              </div>
            )}
            ...
            <div style={S.card}>
              <SectionHeader sectionKey="quiz_mcq" title={T.mcqTitle}
                badge={poolSize>0?uiLang==="en"?`${poolSize} available`:`${poolSize}개 가능`:uiLang==="en"?"No words":uiLang==="en"?"No words":"단어 없음"}/>
              ...
              <div style={S.optionRow}>{[
                ...
                [QUIZ_FILTERS.LEARNING_ONLY, T.learningOnly(learningCount)],
            ...
            {currentBook==="hebrew"&&<div style={{...S.card,border:"1px solid rgba(80,160,120,0.3)"}}>
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"6px"}}>
                ...
                <button onClick={()=>setVariantCatsSave(variantCats.length===VARIANT_CATS.length?[]:VARIANT_CATS.map(c=>c.id))}
                  ...
                  {variantCats.length===VARIANT_CATS.length?T.allDeselect:T.allSelectAll}
                </button>
              </div>
              <div style={S.optionRow}>
              ...
              {(()=>{
                ...
                return(
                  <div style={{display:"flex",gap:"5px",flexWrap:"wrap"}}>
                    {[
                      [QUIZ_FILTERS.ALL, uiLang==="en"?`All (${vAll})`:`전체 (${vAll})`],
                      [QUIZ_FILTERS.LEARNING_ONLY, uiLang==="en"?`📖 Learning (${vLearning})`:`📖 학습중 (${vLearning})`],
                      [QUIZ_FILTERS.HARD_ONLY, uiLang==="en"?`🔥 Hard (${vHard})`:`🔥 어려움 (${vHard})`],
                      [QUIZ_FILTERS.EXCLUDE_MASTERED, uiLang==="en"?`Excl. Mastered (${vExclude})`:`암기 제외 (${vExclude})`],
                    ].map(([val,label])=>(
              ...
              <button style={{...S.btnStart,background:"linear-gradient(135deg,#50c898,#70e8b8)",color:"#0f1a14",...(!variantPoolSize||!variantCats.length?S.btnDisabled:{})}}
                ...
                {uiLang==="en"?`🔀 Start Variant Quiz! (${variantCount===9999?variantPoolSize:Math.min(variantCount,variantPoolSize)})`:`🔀 변형 퀴즈 시작! (${variantCount===9999?variantPoolSize:Math.min(variantCount,variantPoolSize)}문제)`}
              </button>
              <div style={{display:"flex",gap:"6px",marginTop:"8px"}}>
                {[["essay",uiLang==="en"?"✍️ Written":"✍️ 서술형"],["mcq",uiLang==="en"?"🎯 Multiple Choice":"🎯 객관식"]].map(([t,label])=>(
        ...
        {mode===MODES.QUIZ&&q&&(
          <div key={animKey}>
            ...
            <div style={S.progressLabel}><span>{current+1} / {questions.length}</span><span style={S.scoreLabel}>{uiLang==="en"?"Score":"점수"}: {score} / {current+(confirmed?1:0)}</span></div>
              ...
              {confirmed&&(
                <>
                  <div style={{...(selected===q.answer?S.feedbackCorrect:S.feedbackWrong),marginBottom:"6px",padding:"8px 12px",fontSize:"0.88rem"}}>
                    ...
                    {(()=>{const w=words.find(x=>x.id===q.wordId);const st=w?STATUS_CONFIG[w.status]:null;return st?<span style={{marginLeft:6,fontSize:"0.75rem",opacity:0.8}}>{st.emoji} {(uiLang==="en"?st.labelEn:st.labelKo)}</span>:null;})()}
                  </div>
                  {/* 퀴즈 중 상태 분류 버튼 */}
        ...
        {mode===MODES.ESSAY&&eq&&(
          <div key={animKey}>
            ...
            <div style={S.progressLabel}><span>{uiLang==="en"?"✍️":"✍️"} {essayCurrent+1} / {essayQuestions.length}</span><span style={{color:"#9060f0",fontWeight:600}}>정답 {essayResults.filter(r=>r.result!=="wrong").length} / {essayCurrent+(essayConfirmed?1:0)}</span></div>
            ...
            {essayConfirmed&&(()=>{ const last=essayResults[essayResults.length-1]; const w=words.find(x=>x.id===eq.wordId);
              return(<>
                {last?.result==="exact"
                  ?<div style={{...S.feedbackCorrect,flexWrap:"wrap",marginBottom:"6px"}}>{uiLang==="en"?"✅ Correct!":"✅ 정답!"} <SpeakBtn text={eq.hebrewWord} onSpeak={speakOnDemand} muted={muted}/></div>
                  :last?.result==="partial"
                  ?<div style={{...S.feedbackCorrect,background:"rgba(196,160,80,0.15)",borderColor:"rgba(196,160,80,0.3)",color:"#e8c875",flexWrap:"wrap",marginBottom:"6px"}}>{uiLang==="en"?"Partial! Answer:":"부분 정답! 정답:"} <b>{eq.answer}</b> <SpeakBtn text={eq.hebrewWord} onSpeak={speakOnDemand} muted={muted}/></div>
                  :<div style={{...S.feedbackWrong,flexWrap:"wrap",marginBottom:"6px"}}>{uiLang==="en"?"❌ Wrong — Answer:":"❌오답—정답:"} <b>{eq.answer}</b> <SpeakBtn text={eq.hebrewWord} onSpeak={speakOnDemand} muted={muted}/></div>
                }
          ...
          return(
            <div>
              ...
              <div style={S.progressLabel}>
                ...
                <span style={{color:"#50c898",fontWeight:600}}>{uiLang==="en"?"Score":uiLang==="en"?"Exact":uiLang==="en"?"Correct":"정답"} {variantResults.filter(r=>r.correct).length} / {variantCur+(variantConfirmed?1:0)}</span>
              </div>
              ...
              {variantQuizType==="essay"&&(
                <>
                  <input
                    ...
                    placeholder={uiLang==="en"?"Enter Hebrew variant...":"변형을 히브리어로 입력..."}
                  ...
                  {variantConfirmed&&(
                    <div style={{marginBottom:"10px",display:"flex",alignItems:"center",justifyContent:"center",gap:"8px"}}>
                      {lastResult?.correct
                        ?<div style={{...S.feedbackCorrect,padding:"8px 16px"}}>{uiLang==="en"?"✅ Correct!":"✅ 정답!"}</div>
        ...
        {mode===MODES.VARIANT_RESULT&&(
          <div style={S.resultWrap}>
            ...
            <p style={{fontSize:"0.8rem",color:"#50c898",fontWeight:600,marginBottom:"4px"}}>{uiLang==="en"?"🔀 Variant Quiz Results":"🔀 변형 퀴즈 결과"}</p>
            <p style={S.resultMsg}>{variantResults.filter(r=>r.correct).length===variantQuestions.length?(uiLang==="en"?"🎉 Perfect!":"🎉 완벽해요!"):variantResults.filter(r=>r.correct).length>=variantQuestions.length*0.7?"👏 잘했어요!":"📖 틀린 변형을 복습해봐요!"}</p>
            <p style={S.resultPct}>{uiLang==="en"?"Score":"정답률"}: {Math.round(variantResults.filter(r=>r.correct).length/variantQuestions.length*100)}%</p>
            <div style={S.wrongList}>
              <h3 style={{...S.wrongTitle,color:"#50c898"}}>{uiLang==="en"?"📋 All Results":"📋 전체 결과"}</h3>
            ...
            <div className="result-btn-row" style={S.resultBtnRow}>
              <button style={{...S.btnStart,flex:1,background:"linear-gradient(135deg,#50c898,#70e8b8)",color:"#0f1a14"}} onClick={startVariantQuiz}>{uiLang==="en"?"🔄 Try Again":"🔄 다시 풀기"}</button>
              <button style={{...S.btnQuit,flex:1}} onClick={()=>setMode(MODES.LIST)}>{uiLang==="en"?"📚 Wordlist":"📚 단어장으로"}</button>
            </div>
          </div>
        )}
        ...
        {mode===MODES.ESSAY_RESULT&&(
          <div style={S.resultWrap}>
            ...
            <p style={S.resultMsg}>{essayScore===essayQuestions.length?(uiLang==="en"?"🎉 Perfect!":"🎉 완벽해요!"):(essayScore+essayPartial)>=essayQuestions.length*0.7?"👏 잘했어요!":"📖 틀린 단어를 복습해봐요!"}</p>
            <div style={{display:"flex",justifyContent:"center",gap:"20px",marginBottom:"20px",flexWrap:"wrap"}}>
              ...
              <div style={{textAlign:"center",color:"#e8c875"}}><div style={{fontSize:"1.6rem",fontWeight:800}}>{essayPartial}</div><div style={{fontSize:"0.72rem",opacity:0.7}}>{uiLang==="en"?"🟡 Partial":"🟡 부분 정답"}</div></div>
            ...
            <div style={S.wrongList}>
              <h3 style={{...S.wrongTitle,color:"#c4a050"}}>{uiLang==="en"?"📋 All Results":"📋 전체 결과"}</h3>
            ...
            <div className="result-btn-row" style={S.resultBtnRow}>
              <button style={{...S.btnEssayStart,flex:1}} onClick={startEssay}>{uiLang==="en"?"🔄 Try Again":"🔄 다시 풀기"}</button>
              <button style={{...S.btnQuit,flex:1}} onClick={()=>setMode(MODES.LIST)}>{uiLang==="en"?"📚 Wordlist":"📚 단어장으로"}</button>
            </div>
          </div>
        )}
...
const S={
  // ── 배경: #17161C 기반 다크 테마 ──────────────────────────────
  root:{minHeight:"100vh",background:"#17161C",color:"#ffffff",fontFamily:"'Noto Sans KR','Apple SD Gothic Neo',sans-serif",position:"relative",overflow:"hidden",padding:"16px 0 80px"},
  ...
  // ── 배너 ──────────────────────────────────────────────────
  autoSaveBanner:{background:"rgba(34,197,94,0.07)",border:"1px solid rgba(34,197,94,0.18)",borderRadius:"10px",padding:"10px 16px",marginBottom:"14px",fontSize:"0.82rem",color:"#4ade80"},
  // ── 모달 ──────────────────────────────────────────────────
  ...
  modalTA:{width:"100%",height:"160px",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:"10px",color:"#ffffff",padding:"12px",fontSize:"0.82rem",resize:"vertical",outline:"none",fontFamily:"monospace",marginBottom:"12px"},
  ...
  modalPreviewItem:{padding:"5px 0",borderBottom:"1px solid rgba(255,255,255,0.06)",display:"flex",alignItems:"center",fontSize:"0.88rem",gap:"4px"},
  ...
  subtitle:{margin:"2px 0 0",fontSize:"0.67rem",color:"rgba(255,255,255,0.35)"},
  statsRow:{display:"flex",gap:"5px"},
  statBadge:{borderRadius:"8px",padding:"5px 9px",fontSize:"0.78rem",fontWeight:700},
  // ── 카드류 ──────────────────────────────────────────────────
  ioCard:{background:"#1E1D24",border:"1px solid rgba(255,255,255,0.08)",borderRadius:"14px",padding:"14px 16px",marginBottom:"12px",boxShadow:"0 2px 8px rgba(0,0,0,0.2)"},
  ioTitle:{fontSize:"0.88rem",fontWeight:600,color:"#c4a050",marginBottom:"3px"},
  ioSub:{fontSize:"0.74rem",color:"rgba(255,255,255,0.35)"},
  ...
  card:{background:"#1E1D24",borderRadius:"14px",border:"1px solid rgba(255,255,255,0.08)",padding:"16px",marginBottom:"12px",boxShadow:"0 2px 8px rgba(0,0,0,0.2)"},
  ...
  sliderLabel:{fontSize:"0.8rem",color:"rgba(255,255,255,0.35)",flexShrink:0},
  ...
  questionCard:{background:"#1E1D24",borderRadius:"20px",border:"1px solid rgba(255,255,255,0.1)",padding:"28px 20px",textAlign:"center",marginBottom:"16px",boxShadow:"0 8px 32px rgba(0,0,0,0.4)"},
  ...
  feedbackCorrect:{display:"flex",alignItems:"center",justifyContent:"center",gap:"8px",flexWrap:"wrap",textAlign:"center",padding:"12px",borderRadius:"10px",background:"rgba(74,222,128,0.12)",border:"1px solid rgba(74,222,128,0.3)",color:"#86efac",fontWeight:600,marginBottom:"14px",fontSize:"0.95rem"},
  feedbackWrong:{display:"flex",alignItems:"center",justifyContent:"center",gap:"8px",flexWrap:"wrap",textAlign:"center",padding:"12px",borderRadius:"10px",background:"rgba(239,68,68,0.12)",border:"1px solid rgba(239,68,68,0.3)",color:"#fca5a5",fontWeight:600,marginBottom:"14px",fontSize:"0.95rem"},
  ...
  resultStatLabel:{fontSize:"0.7rem",opacity:0.6,marginTop:"2px"},
