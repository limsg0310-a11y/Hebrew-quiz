...
const syncToCloud=async(wordsToSync)=>{
    if(!user) return;
    setSyncing(true);
    // 책별로 분리된 필드에 저장 (merge:true로 다른 책 데이터 보존)
    const fieldKey=currentBook&&currentBook!=="hebrew"?`words_${currentBook}`:"words";
    try{ await setDoc(doc(fbDb,"users",user.uid),{[fieldKey]:wordsToSync,updatedAt:new Date().toISOString()},{merge:true}); }
...
if(snap.exists()){
const sd=snap.data(); const bookField=currentBook&&currentBook!=="hebrew"?`words_${currentBook}`:"words"; const cloud=sd[bookField]||sd.words;
...
else if(localWords.length>cloud.length+5){setWordsRaw(localWords);saveWords(localWords);setDoc(doc(fbDb,"users",u.uid),{words:localWords,updatedAt:new Date().toISOString()},{merge:true});localStorage.setItem(syncKey,"1");}
...
<div style={{width:"50px",height:"50px",borderRadius:"12px",overflow:"hidden",flexShrink:0,border:`1px solid ${theme==="light"?"rgba(196,160,80,0.35)":"rgba(196,160,80,0.4)"}`,boxShadow:"0 2px 10px rgba(0,0,0,0.2)",background:"transparent",display:"flex",alignItems:"center",justifyContent:"center"}}>
<img src={DOVE_IMG} alt="logo" style={{width:"55%",height:"55%",objectFit:"contain",margin:"auto",display:"block",filter:theme==="dark"?"brightness(1.5) contrast(1.2)":"none"}}/>
</div>
<div>
...
<button onClick={async()=>{
  try{
    const local=JSON.parse(localStorage.getItem("hebrew_quiz_words")||"[]");
    if(!local.length){alert("localStorage에 단어가 없습니다.");return;}
    if(!window.confirm("localStorage의 "+local.length+"개 단어를 Firebase에 복구할까요?"))return;
    const {doc,setDoc}=await import("firebase/firestore");
    await setDoc(doc(fbDb,"users",user.uid),{words:local,updatedAt:new Date().toISOString()},{merge:true});
    setWordsRaw(local);saveWords(local,"hebrew");
    alert("완료! "+local.length+"개 단어가 복구되었습니다.");
  }catch(e){alert("오류: "+e.message);}
}} style={{...S.btnAdd,width:"100%",marginBottom:"8px",background:"rgba(100,180,100,0.15)",borderColor:"rgba(100,180,100,0.4)",color:"#60c060"}}>🔄 단어 복구 (localStorage → Firebase)</button>
...
{activeTab==="ai"&&(()=>{
  const doAsk=async()=>{
    ...
    try{
      // 시스템 프롬프트
      const systemMsg={role:"system",content:"당신은 히브리어 학습 도우미입니다. 반드시 아래 규칙을 지키세요. [언어 규칙] 한국어로만 답변하세요. 히브리어 단어는 히브리어 문자(예: שָׁלוֹם)로만 쓰세요. 절대로 한자(중국어), 러시아어, 일본어, 아랍어 등 다른 언어 문자를 쓰지 마세요. 한국어와 히브리어 문자 외의 문자는 전혀 사용하지 마세요. [내용 규칙] 성경 구절은 개역개정 번역을 사용하세요. 히브리어 원문을 항상 히브리어 문자로 함께 표기하세요. 답변은 명확하고 간결하게 하세요."};
