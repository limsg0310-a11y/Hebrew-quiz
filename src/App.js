...
export default function HebrewQuiz() {
  ...
  useEffect(()=>{
    ...
    chatUnsubRef.current=onSnapshot(q2,snap=>{
      ...
      const othersCount=msgs.filter(m=>m.uid!==user.uid).length;
      if(activeTabRef.current==="chat"){
        ...
        // 채팅 탭 밖에서는 새 메시지만 카운트
        const lastIdx=msgs.findIndex(m=>m.id===lastReadMsgIdRef.current);
        const newUnread=lastIdx===-1?msgs.filter(m=>m.uid!==user.uid).length:msgs.slice(lastIdx+1).filter(m=>m.uid!==user.uid).length;
