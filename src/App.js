...
export default function HebrewQuiz() {
  ...
  useEffect(()=>{
    ...
    chatUnsubRef.current=onSnapshot(q2,snap=>{
      ...
      // 전체 메시지 수 기준으로 알림 계산 (본인 제외)
      const othersCount=msgs.filter(m=>m.uid!==user.uid).length;
      if(activeTab!=="chat"){
        const newUnread=Math.max(0,othersCount-lastReadChatCountRef.current);
        setUnreadChat(newUnread);
      } else {
        // 채팅 탭 열려있으면 항상 읽음 처리
        lastReadChatCountRef.current=othersCount;
        setUnreadChat(0);
  ...
  useEffect(()=>{
    if(activeTab==="chat"&&user){
      // 탭 진입 시 현재 메시지 수를 읽음으로 표시
      const othersCount=globalMessages.filter(m=>m.uid!==user.uid).length;
      lastReadChatCountRef.current=othersCount;
      setUnreadChat(0);
