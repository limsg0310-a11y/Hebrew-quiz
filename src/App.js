...
export default function HebrewQuiz() {
  ...
  const sendGlobalMessage=async()=>{
    ...
    if(editingMsgId){
      // 수정 모드
      await updateDoc(doc(fbDb,"globalChat",editingMsgId),{text:chatInput.trim(),edited:true,editedAt:serverTimestamp()});
      setEditingMsgId(null);setEditingMsgText("");setChatInput("");
    } else {
      const text=chatInput.trim(); setChatInput("");
      await addDoc(collection(fbDb,"globalChat"),{uid:user.uid,displayName:user.displayName,photoURL:user.photoURL,text,createdAt:serverTimestamp(),edited:false});
    }
  };
  ...
  const cancelEdit=()=>{setEditingMsgId(null);setEditingMsgText("");setChatInput("");};
  ...
  useEffect(()=>{
    ...
    const play=async()=>{
      ...
      // 2) 뜻이 있으면 1.2초 후 뜻 발음 (UI 언어에 맞게)
      ...
      if(meaning){
        ...
        try{
          const book=BOOKS.find(b=>b.id===currentBook)||BOOKS[0];
          // 뜻은 한국어/영어이므로 ko-KR 또는 en-US로
          const meaningLang=(/[ㄱ-ㅎ가-힣]/.test(meaning))?"ko-KR":"en-US";
          const meaningName=meaningLang==="ko-KR"?"ko-KR-Standard-A":"en-US-Standard-C";
          if(apiKey){try{await googleTTS(meaning,apiKey,meaningLang,meaningName,1.0);return;}catch{}}
          browserTTS(meaning,meaningLang,1.0);
        }catch{}
      }
...
`}</style>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:800,display:"flex",alignItems:"flex-end"}} onClick={()=>setBottomMenuWord(null)}>
          <div onClick={e=>e.stopPropagation()} style={{width:"100%",background:"#1E1D24",borderRadius:"20px 20px 0 0",border:"1px solid rgba(255,255,255,0.1)",padding:"20px 16px 32px",boxShadow:"0 -8px 40px rgba(0,0,0,0.6)"}}>
            ...
            <div style={{display:"flex",alignItems:"flex-start",gap:"10px",marginBottom:"12px",padding:"12px 14px",background:"rgba(255,255,255,0.04)",borderRadius:"12px"}}>
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{display:"flex",gap:"5px",flexWrap:"wrap"}}>
                  ...
                  {(bottomMenuWord.variants||[]).length>0&&<span style={{fontSize:"0.65rem",color:"#9060f0",background:"rgba(100,80,200,0.12)",border:"1px solid rgba(100,80,200,0.2)",borderRadius:"5px",padding:"2px 7px"}}>🔀 {bottomMenuWord.variants.length}</span>}
                  {(bottomMenuWord.wrongCount||0)>0&&<span style={{fontSize:"0.65rem",color:"#f07050",background:"rgba(200,60,60,0.12)",border:"1px solid rgba(200,60,60,0.25)",borderRadius:"5px",padding:"2px 7px"}}>❌ {bottomMenuWord.wrongCount}</span>}
                </div>
              </div>
              <span style={{fontSize:"0.65rem",color:STATUS_CONFIG[bottomMenuWord.status].color,background:STATUS_CONFIG[bottomMenuWord.status].bg,border:`1px solid ${STATUS_CONFIG[bottomMenuWord.status].border}`,borderRadius:"5px",padding:"3px 8px",flexShrink:0}}>
                {STATUS_CONFIG[bottomMenuWord.status].emoji} {uiLang==="ko"?STATUS_CONFIG[bottomMenuWord.status].labelKo:STATUS_CONFIG[bottomMenuWord.status].labelEn}
              </span>
            </div>
            {/* 상태 변경 */}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"6px",marginBottom:"10px"}}>
              {["learning","hard","mastered"].map(s=>{const sc=STATUS_CONFIG[s];return(
                <button key={s} onClick={()=>{setManualStatus(bottomMenuWord.id,s);setBottomMenuWord(null);}} style={{padding:"10px 4px",borderRadius:"10px",border:`1px solid ${bottomMenuWord.status===s?sc.border:"rgba(255,255,255,0.1)"}`,background:bottomMenuWord.status===s?sc.bg:"rgba(255,255,255,0.04)",color:bottomMenuWord.status===s?sc.color:"rgba(255,255,255,0.5)",cursor:"pointer",fontSize:"0.82rem",fontWeight:600}}>
                  {sc.emoji} {uiLang==="ko"?sc.labelKo:sc.labelEn}
                </button>
              );})}
            </div>
            {/* 액션 버튼들 */}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"6px"}}>
              ...
              <button onClick={()=>{openVariantModal(bottomMenuWord);setBottomMenuWord(null);}} style={{padding:"11px",borderRadius:"10px",border:"1px solid rgba(100,80,200,0.3)",background:"rgba(100,80,200,0.1)",color:"#9060f0",cursor:"pointer",fontSize:"0.85rem",fontWeight:600}}>🔀 {T.variantTabEdit}</button>
              {wallets.length>0&&<button onClick={()=>{setWalletPickWord(bottomMenuWord.id);setBottomMenuWord(null);}} style={{padding:"11px",borderRadius:"10px",border:"1px solid rgba(196,160,80,0.25)",background:"rgba(196,160,80,0.08)",color:"#c4a050",cursor:"pointer",fontSize:"0.85rem",fontWeight:600}}>📚 {T.addToWordbook}</button>}
              <button onClick={()=>{if(window.confirm(T.deleteConfirm))deleteWord(bottomMenuWord.id);setBottomMenuWord(null);}} style={{padding:"11px",borderRadius:"10px",border:"1px solid rgba(200,60,60,0.3)",background:"rgba(200,60,60,0.1)",color:"#f07070",cursor:"pointer",fontSize:"0.85rem",fontWeight:600,gridColumn:wallets.length>0?"auto":"1/-1"}}>{T.deleteBtn}</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={S.container}>
        {/* ── 헤더 ── */}
        <header style={{...S.header,flexDirection:"column",gap:"8px",padding:"12px 14px"}}>
          {/* 1행: 로고 + 앱 이름 */}
          <div style={{display:"flex",alignItems:"center",gap:"10px",width:"100%"}}>
            <span style={S.logo}>אב</span>
            <div>
              <h1 style={{...S.title,fontSize:"1.05rem",whiteSpace:"nowrap"}}>
                {uiLang==="ko"?"히브리어 단어 퀴즈":"Hebrew Quiz"}
              </h1>
              <div style={{fontSize:"0.6rem",color:ttsReady?"#60c880":"#f07050"}}>{ttsReady?T.ttsHeader(true):T.ttsHeader(false)}</div>
            </div>
          </div>
          {/* 2행: 통계 + 프로필 + 언어 */}
            ...
            <div style={{display:"flex",alignItems:"center",gap:"6px"}}>
              {user?(
                <div style={{display:"flex",alignItems:"center",gap:"5px"}}>
                  ...
                  {syncing&&<span style={{fontSize:"0.6rem",color:"rgba(255,255,255,0.35)"}}>{T.saving}</span>}
                </div>
              ):(
                      ...
                      <div key={w.id} style={{...S.wordItem,borderColor:selectedIds.has(w.id)?"rgba(200,60,60,0.5)":st.border,background:selectedIds.has(w.id)?"rgba(200,60,60,0.08)":undefined}}>
                        ...
                        <div style={{flex:1,minWidth:0,overflow:"hidden"}}>
                          <div style={{display:"flex",alignItems:"center",gap:"5px",flexWrap:"nowrap"}}>
                            ...
                            <span style={{fontSize:"0.6rem",color:st.color,flexShrink:0}}>{st.emoji}</span>
                          </div>
                      ...
                      <div style={{display:"flex",flexDirection:"column",flex:1,minHeight:0}}>
                        ...
                        {longPressMsg&&(
                          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:900,display:"flex",alignItems:"flex-end"}} onClick={()=>setLongPressMsg(null)}>
                            <div onClick={e=>e.stopPropagation()} style={{width:"100%",background:"#1E1D24",borderRadius:"20px 20px 0 0",padding:"20px 16px 32px",border:"1px solid rgba(255,255,255,0.1)"}}>
                              ...
                              <div style={{display:"flex",gap:"8px"}}>
                                <button onClick={()=>startEditMsg(longPressMsg)} style={{flex:1,padding:"12px",borderRadius:"10px",background:"rgba(196,160,80,0.15)",border:"1px solid rgba(196,160,80,0.3)",color:"#c4a050",cursor:"pointer",fontWeight:600}}>✏️ {uiLang==="ko"?"수정":"Edit"}</button>
                                <button onClick={()=>deleteGlobalMessage(longPressMsg.id)} style={{flex:1,padding:"12px",borderRadius:"10px",background:"rgba(200,60,60,0.15)",border:"1px solid rgba(200,60,60,0.3)",color:"#f07070",cursor:"pointer",fontWeight:600}}>🗑️ {uiLang==="ko"?"삭제":"Delete"}</button>
                              </div>
                            </div>
                          </div>
                        )}
                        <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:"8px",paddingBottom:"8px"}} ref={el=>{if(el&&!el._inited){el.scrollTop=el.scrollHeight;el._inited=true;}}}>
                          {globalMessages.map((msg,mi)=>{
                            ...
                            const isUnread=!isMe&&mi>=lastReadChatCountRef.current;
                            return(
                              <div key={msg.id}>
                                {isUnread&&mi===lastReadChatCountRef.current&&globalMessages.filter((m,i)=>!m.uid!==user.uid&&i>=lastReadChatCountRef.current).length>0&&(
                        ...
                        <div style={{paddingTop:"8px",borderTop:"1px solid rgba(255,255,255,0.08)"}}>
                          {editingMsgId&&(
                            <div style={{display:"flex",alignItems:"center",gap:"8px",padding:"6px 10px",background:"rgba(196,160,80,0.1)",borderRadius:"8px",marginBottom:"6px",fontSize:"0.75rem",color:"#c4a050"}}>
                              ...
                              <button onClick={cancelEdit} style={{background:"none",border:"none",color:"#f07050",cursor:"pointer",fontSize:"0.8rem"}}>✕</button>
                            </div>
                          )}
                          <div style={{display:"flex",gap:"8px"}}>
