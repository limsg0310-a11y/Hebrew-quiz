...
const UI_TEXT = {
  ko: {
    ...
    variantTypeSelect:"변형 유형 선택", allDeselect:"전체 해제", allSelectAll:"전체 선택",
...
export default function HebrewQuiz() {
  ...
  // ── 전체 반복 듣기 ──
  const [listenAll,setListenAll]=useState({active:false,paused:false,index:0,words:[],walletId:null});
  const listenAllRef=useRef(null);
  const listenAllStopFn=()=>{clearTimeout(listenAllRef.current);setListenAll({active:false,paused:false,index:0,words:[],walletId:null});window.speechSynthesis?.cancel();};
  const listenAllStart=(targetWords,walletId=null)=>{if(!targetWords.length)return;listenAllStopFn();setListenAll({active:true,paused:false,index:0,words:targetWords,walletId});};
  ...
  const handleMerge=(choice)=>{
    ...
    if(choice==="cloud"){setWordsRaw(pendingCloudWords);saveWords(pendingCloudWords);showToast("☁️ 클라우드 단어장으로 교체했어요!");}
    else if(choice==="local"){const local=loadWords();if(user)setDoc(doc(fbDb,"users",user.uid),{words:local,updatedAt:new Date().toISOString()});showToast("💾 기기 단어장을 유지했어요!");}
    else{const local=loadWords();const hs=new Set(pendingCloudWords.map(w=>w.hebrew));const merged=[...pendingCloudWords,...local.filter(w=>!hs.has(w.hebrew))];setWordsRaw(merged);saveWords(merged);if(user)setDoc(doc(fbDb,"users",user.uid),{words:merged,updatedAt:new Date().toISOString()});showToast(`☁️ 병합 완료! 총 ${merged.length}개 단어`);}
  ...
  const searchByRoot=async()=>{
    ...
    try{
      ...
      if(!data.results?.length){setRootSearchError("검색 결과가 없어요.");return;}
  ...
  const addSelectedRootWords=()=>{
    ...
    showToast(`✅ ${newWords.length}개 단어를 추가했어요!`);setRootSelected(new Set());setRootSearchResults([]);setRootSearchInput("");
  };
  ...
  const searchWordByMeaning=async()=>{
    ...
    try{
      ...
      if(!data.results?.length){setWordSearchError(`"${q2}" 검색 결과가 없어요.`);return;}
  ...
  const copyToClipboard=async()=>{
    ...
    catch{const ta=document.createElement("textarea");ta.value=text;document.body.appendChild(ta);ta.select();document.execCommand("copy");document.body.removeChild(ta);showToast("📋 복사됐어요!");}
  };
  ...
  const importFromText=()=>{
    try{
      ...
      const imported=raw.map(w=>({id:Date.now()+Math.random(),hebrew:(w.hebrew||"").trim(),meaning:(w.meaning||"").trim(),status:["learning","mastered","hard"].includes(w.status)?w.status:"learning",streak:w.streak||0,wrongCount:w.wrongCount||0})).filter(w=>w.hebrew&&w.meaning);
      if(!imported.length){showToast("불러올 단어가 없어요.","err");return;}
      setImportPreview({words:imported,fileName:"클립보드에서"});setShowPasteImport(false);setPasteText("");
  ...
  const confirmImport=(merge)=>{
    ...
    if(merge){const ex=new Set(words.map(w=>w.hebrew));const newOnes=importPreview.words.filter(w=>!ex.has(w.hebrew));setWords(ws=>[...newOnes,...ws]);setPage(0);showToast(`📥 ${newOnes.length}개 추가!`);}
  ...
  useEffect(()=>{
    ...
    const {words:lw,index}=listenAll;
    if(index>=lw.length){setListenAll(l=>({...l,active:false}));return;}
    const w=lw[index];
    const play=async()=>{
      try{await speakOnDemand(w.hebrew||w);}catch{}
      listenAllRef.current=setTimeout(()=>{setListenAll(l=>l.active&&!l.paused?{...l,index:l.index+1}:l);},2200);
    };
    play();
    return()=>clearTimeout(listenAllRef.current);
  },[listenAll.active,listenAll.paused,listenAll.index]);
...
`}</style>
      ...
      {showMergeModal&&pendingCloudWords&&(
        <div style={S.modalOverlay}><div style={S.modal}>
          ...
          <p style={S.modalSub}>기기와 클라우드 단어장이 모두 있어요. 어떻게 할까요?</p>
          <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
            <button style={{...S.btnMerge,padding:"12px"}} onClick={()=>handleMerge("merge")}>🔀 합치기</button>
            <button style={{...S.btnReplace,padding:"12px"}} onClick={()=>handleMerge("cloud")}>☁️ 클라우드 사용 ({pendingCloudWords.length}개)</button>
            <button style={{...S.btnCancel2,padding:"12px"}} onClick={()=>handleMerge("local")}>💾 기기 단어 유지</button>
          </div>
        </div></div>
      )}
          ...
          <div style={{...S.modalOverlay,alignItems:"flex-start",paddingTop:"20px",overflowY:"auto"}}>
            <div style={{...S.modal,maxWidth:"600px",maxHeight:"90vh",overflowY:"auto"}}>
              ...
              <div style={{display:"flex",gap:"6px",marginBottom:"14px"}}>
                {[["편집",false],["보기","view"],["붙여넣기","paste"]].map(([lbl,val])=>(
              ...
              {variantPasteMode==="view"&&(()=>{
                ...
                if(!Object.keys(v).length) return <div style={{textAlign:"center",color:"rgba(255,255,255,0.35)",padding:"30px 0"}}>변형 데이터가 없어요.</div>;
                return(
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:800,display:"flex",alignItems:"flex-end"}} onClick={()=>setBottomMenuWord(null)}>
          <div onClick={e=>e.stopPropagation()} style={{width:"100%",background:"#1E1D24",borderRadius:"20px 20px 0 0",border:"1px solid rgba(255,255,255,0.1)",padding:"20px 16px 32px",boxShadow:"0 -8px 40px rgba(0,0,0,0.6)"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"6px"}}>
              <button onClick={()=>{speakOnDemand(bottomMenuWord.hebrew);}} style={{padding:"11px",borderRadius:"10px",border:"1px solid rgba(196,160,80,0.3)",background:"rgba(196,160,80,0.1)",color:"#c4a050",cursor:"pointer",fontSize:"0.85rem",fontWeight:600}}>🔈 발음 듣기</button>
              ...
              <button onClick={()=>{if(window.confirm("삭제할까요?"))deleteWord(bottomMenuWord.id);setBottomMenuWord(null);}} style={{padding:"11px",borderRadius:"10px",border:"1px solid rgba(200,60,60,0.3)",background:"rgba(200,60,60,0.1)",color:"#f07070",cursor:"pointer",fontSize:"0.85rem",fontWeight:600,gridColumn:wallets.length>0?"auto":"1/-1"}}>🗑️ 삭제</button>
            </div>
          </div>
        </div>
      )}
      ...
      {listenAll.active&&(
        <div style={{position:"fixed",bottom:isQuizActive?0:60,left:0,right:0,zIndex:190,background:"linear-gradient(135deg,#1a1820,#22202e)",borderTop:"1px solid rgba(196,160,80,0.3)",padding:"8px 16px",display:"flex",alignItems:"center",gap:"10px",boxShadow:"0 -4px 20px rgba(0,0,0,0.4)"}}>
          ...
          <span style={{fontSize:"0.7rem",color:"rgba(255,255,255,0.5)",flexShrink:0}}>{listenAll.index}/{listenAll.words.length}</span>
      ...
      <div style={S.container}>
        ...
        <header style={S.header}>
          <div style={S.headerLeft}>
            ...
            <div>
              ...
              <div style={{fontSize:"0.65rem",color:ttsReady?"#60c880":"#f07050"}}>{ttsReady?"🔊 Google TTS":"⚠️ Browser TTS"}</div>
            </div>
          </div>
          ...
          <div>
                    ...
                    <>
                      ...
                      <div style={{display:"flex",gap:"6px",justifyContent:"center",flexWrap:"wrap"}}>
                        {(()=>{const w=words.find(x=>x.id===q.wordId);if(!w)return null;return(<>
                          {w.status!=="hard"&&<button onClick={()=>setManualStatus(q.wordId,"hard")} style={{padding:"3px 10px",borderRadius:"6px",background:"rgba(200,80,60,0.15)",border:"1px solid rgba(200,80,60,0.4)",color:"#f07050",fontSize:"0.72rem",cursor:"pointer"}}>🔥 어려움</button>}
                          {w.status!=="mastered"&&<button onClick={()=>setManualStatus(q.wordId,"mastered")} style={{padding:"3px 10px",borderRadius:"6px",background:"rgba(60,180,100,0.15)",border:"1px solid rgba(60,180,100,0.4)",color:"#60c880",fontSize:"0.72rem",cursor:"pointer"}}>✅ 암기완료</button>}
                          {w.status!=="learning"&&<button onClick={()=>setManualStatus(q.wordId,"learning")} style={{padding:"3px 10px",borderRadius:"6px",background:"rgba(120,120,160,0.15)",border:"1px solid rgba(120,120,160,0.3)",color:"#9090b0",fontSize:"0.72rem",cursor:"pointer"}}>📖 학습중</button>}
                        </>);})()}
                      </div>
                    </>
                  )}
              ...
              <div key={animKey}>
                ...
                {essayConfirmed&&(()=>{
                  ...
                  return(<>
                    ...
                    :last?.result==="partial"?<div style={{...S.feedbackCorrect,background:"rgba(196,160,80,0.15)",borderColor:"rgba(196,160,80,0.3)",color:"#e8c875",flexWrap:"wrap",marginBottom:"6px"}}>부분 정답! 정답: <b>{eq.answer}</b> <SpeakBtn text={eq.hebrewWord} onSpeak={speakOnDemand} muted={muted}/></div>
                    ...
                    {w&&<div style={{display:"flex",gap:"6px",justifyContent:"center",flexWrap:"wrap",marginBottom:"8px"}}>
                      {w.status!=="mastered"&&<button onClick={()=>setManualStatus(eq.wordId,"mastered")} style={{padding:"3px 10px",borderRadius:"6px",background:"rgba(60,180,100,0.15)",border:"1px solid rgba(60,180,100,0.4)",color:"#60c880",fontSize:"0.72rem",cursor:"pointer"}}>✅ 암기완료</button>}
                      {w.status!=="hard"&&<button onClick={()=>setManualStatus(eq.wordId,"hard")} style={{padding:"3px 10px",borderRadius:"6px",background:"rgba(200,80,60,0.15)",border:"1px solid rgba(200,80,60,0.4)",color:"#f07050",fontSize:"0.72rem",cursor:"pointer"}}>🔥 어려움</button>}
                    </div>}
                  </>);
                })()}
                <div className="quiz-btn-row" style={S.quizBtnRow}>
            ...
            {mode===MODES.RESULT&&(
              <div style={S.resultWrap}>
                ...
                <p style={S.resultMsg}>{score===questions.length?"🎉 완벽!":score>=questions.length*0.7?"👏 잘했어요!":"📖 다시 도전!"}</p>
            ...
            {mode===MODES.VARIANT_RESULT&&(
              <div style={S.resultWrap}>
                ...
                <p style={S.resultMsg}>{variantResults.filter(r=>r.correct).length===variantQuestions.length?"🎉 완벽!":"🔀 변형 퀴즈 완료!"}</p>
                <div style={S.wrongList}>
          ...
          <div style={{paddingBottom:"80px"}}>
            ...
            {activeTab==="list"&&(
              <div>
                ...
                {rootGroupView&&(()=>{
                  ...
                  if(!roots.length) return <div style={S.emptyMsg}>어근 정보가 있는 단어가 없어요.</div>;
                  return(
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"6px",flexWrap:"wrap",gap:"6px"}}>
                  <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
                    ...
                    {selectedIds.size>0&&(<>
                      ...
                      <button style={{...S.scrollBtn,background:"rgba(200,60,60,0.15)",borderColor:"rgba(200,60,60,0.4)",color:"#f08080",fontSize:"0.75rem"}} onClick={()=>{if(window.confirm(`${selectedIds.size}개를 삭제할까요?`)){setWords(ws=>ws.filter(w=>!selectedIds.has(w.id)));setSelectedIds(new Set());}}}>
                      ...
                      <div key={w.id} style={{...S.wordItem,borderColor:selectedIds.has(w.id)?"rgba(200,60,60,0.5)":st.border,background:selectedIds.has(w.id)?"rgba(200,60,60,0.08)":undefined}}>
                        ...
                        <div style={{flex:1,minWidth:0}}>
                          ...
                          <div style={{display:"flex",alignItems:"center",gap:"5px",flexWrap:"wrap",marginTop:"2px"}}>
                            <span style={S.wordMean}>{w.meaning||<span style={{color:"rgba(255,255,255,0.18)",fontStyle:"italic"}}>뜻 없음</span>}</span>
                ...
                {pageSize!==9999&&totalPages>1&&(
                  <div style={{display:"flex",justifyContent:"center",gap:"6px",marginBottom:"14px",flexWrap:"wrap"}}>
                    <button style={{...S.scrollBtn,...(page===0?{opacity:0.3}:{})}} onClick={()=>page>0&&setPage(p=>p-1)} disabled={page===0}>← 이전</button>
                    ...
                    <button style={{...S.scrollBtn,...(page===totalPages-1?{opacity:0.3}:{})}} onClick={()=>page<totalPages-1&&setPage(p=>p+1)} disabled={page===totalPages-1}>다음 →</button>
                  </div>
                )}
              </div>
            )}
              ...
              <div>
                ...
                {addSubView==="form"&&(
                  <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
                    <div style={S.card}>
                      {editId!==null&&<div style={{fontSize:"0.68rem",color:"#c4a050",letterSpacing:"1.5px",textTransform:"uppercase",marginBottom:"10px",fontWeight:700}}>✏️ 수정 중</div>}
                ...
                {addSubView==="reverso"&&(
                  <div style={S.card}>
                    <div style={{fontSize:"0.82rem",fontWeight:600,color:"#50c898",marginBottom:"12px"}}>🔍 Reverso 동사변형 가져오기</div>
                    <div style={{display:"flex",gap:"8px",marginBottom:"12px"}}>
                      ...
                      <button onClick={searchPealim} disabled={pealimLoading} style={{...S.btnAdd,minWidth:"60px",opacity:pealimLoading?0.6:1}}>{pealimLoading?"…":"검색"}</button>
                    </div>
                    <div style={{display:"flex",gap:"6px",flexWrap:"wrap",marginBottom:"12px"}}>
                      {[["לְדַבֵּר","말하다"],["לָלֶכֶת","가다"],["לֶאֱכֹל","먹다"],["לִכְתּוֹב","쓰다"]].map(([verb,hint])=>(
                      ...
                      <div>
                        <div style={{background:"rgba(80,160,120,0.08)",border:"1px solid rgba(80,160,120,0.2)",borderRadius:"10px",padding:"14px",marginBottom:"10px"}}>
                          <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"10px"}}>
                            ...
                            <span style={{fontSize:"0.72rem",background:"rgba(80,160,120,0.2)",padding:"3px 10px",borderRadius:"6px",color:"#50c898"}}>{Object.keys(pealimPreview.variants||{}).length}개 변형</span>
                          </div>
                          <input value={pealimPreview.meaning||""} onChange={e=>setPealimPreview(p=>({...p,meaning:e.target.value}))} style={{...S.input,marginBottom:"10px"}} placeholder="뜻 입력 *필수"/>
                          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"2px",maxHeight:"200px",overflowY:"auto"}}>
                            {Object.entries(pealimPreview.variants||{}).filter(([,f])=>f).map(([tid,form])=>(
                              <div key={tid} onClick={()=>speakOnDemand(form)} style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"5px 3px",background:"rgba(255,255,255,0.03)",borderRadius:"6px",cursor:"pointer"}}>
                                <span style={{color:"rgba(255,255,255,0.4)",fontSize:"0.55rem"}}>{tid}</span>
                                <span style={{fontFamily:"Arial",direction:"rtl",color:"#f0ece0",fontSize:"0.9rem",fontWeight:600}}>{form}</span>
                              </div>
                            ))}
                          </div>
                        </div>
                        <button onClick={addFromPealim} style={{...S.btnMerge,width:"100%",background:"linear-gradient(135deg,#50c898,#70e8b8)",color:"#0f1a14",padding:"12px",marginBottom:"6px"}}>✅ 단어장에 추가</button>
                        <button onClick={()=>setPealimPreview(null)} style={{...S.btnCancel2,width:"100%"}}>← 다시 검색</button>
                      </div>
                    )}
                    ...
                    {rootSearchResults.length>0&&(
                      <div>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px"}}>
                          <span style={{fontSize:"0.78rem",color:"rgba(255,255,255,0.5)"}}>{rootSearchResults.length}개 결과</span>
                          <div style={{display:"flex",gap:"6px"}}>
                            ...
                            {rootSelected.size>0&&<button onClick={addSelectedRootWords} style={{padding:"5px 12px",borderRadius:"8px",background:"linear-gradient(135deg,#c4a050,#e8c875)",border:"none",color:"#1a1820",fontWeight:700,cursor:"pointer",fontSize:"0.8rem"}}>✅ {rootSelected.size}개 추가</button>}
                          </div>
                        </div>
                          ...
                          {rootSearchResults.map((r,i)=>{
                            ...
                            return(
                              <div key={i} onClick={()=>{if(already)return;setRootSelected(s=>{const n=new Set(s);n.has(i)?n.delete(i):n.add(i);return n;});}} style={{display:"flex",gap:"8px",alignItems:"center",padding:"8px 12px",borderRadius:"10px",background:isSel?"rgba(196,160,80,0.1)":"rgba(255,255,255,0.03)",border:`1px solid ${isSel?"rgba(196,160,80,0.4)":already?"rgba(255,255,255,0.05)":"rgba(255,255,255,0.08)"}`,cursor:already?"default":"pointer",opacity:already?0.5:1}}>
                                ...
                                {already&&<span style={{fontSize:"0.65rem",color:"#50c898"}}>✓ 있음</span>}
                              </div>
                            );
                          })}
                        </div>
                ...
                {addSubView==="meaning"&&(
                  <div style={S.card}>
                    ...
                    <p style={{fontSize:"0.78rem",color:"rgba(255,255,255,0.4)",marginBottom:"10px"}}>한국어 또는 영어로 입력하면 히브리어 단어를 찾아줘요. 예: 사랑, love</p>
                    <div style={{display:"flex",gap:"8px",marginBottom:"10px"}}>
                    ...
                    {wordSearchResults.length>0&&(
                      <div>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px"}}>
                          <span style={{fontSize:"0.78rem",color:"rgba(255,255,255,0.5)"}}>{wordSearchResults.length}개 결과</span>
                          <div style={{display:"flex",gap:"6px"}}>
                            ...
                            {wordSearchSelected.size>0&&<button onClick={addSelectedWordSearch} style={{padding:"5px 12px",borderRadius:"8px",background:"linear-gradient(135deg,#c4a050,#e8c875)",border:"none",color:"#1a1820",fontWeight:700,cursor:"pointer",fontSize:"0.8rem"}}>✅ {wordSearchSelected.size}개 추가</button>}
                          </div>
                        </div>
                        <div style={{display:"flex",flexDirection:"column",gap:"5px",maxHeight:"320px",overflowY:"auto"}}>
                ...
                {addSubView==="batch"&&(
                  <div style={S.card}>
                    <div style={{fontSize:"0.82rem",fontWeight:600,color:"#60c880",marginBottom:"8px"}}>📝 텍스트 일괄 추가</div>
                    ...
                    <button onClick={importFromBatch} style={{...S.btnAdd,width:"100%",padding:"12px"}}>단어 추가하기</button>
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab==="quiz"&&(
              <div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"8px",marginBottom:"16px"}}>
                  {[{key:"mcq",icon:"🎯",color:"#c4a050",title:"객관식",cnt:poolSize},{key:"essay",icon:"✍️",color:"#9060f0",title:"서술형",cnt:essayPoolSize},{key:"variant",icon:"🔀",color:"#50c898",title:"변형 퀴즈",cnt:variantPoolSize}].map(qt=>(
                    <div key={qt.key} style={{background:"#1E1D24",border:`1px solid ${qt.color}30`,borderRadius:"14px",padding:"14px 10px",textAlign:"left",cursor:"pointer"}} onClick={()=>document.getElementById(`quiz-${qt.key}`)?.scrollIntoView({behavior:"smooth"})}>
                      ...
                      <div style={{fontSize:"0.62rem",color:qt.color,fontFamily:"monospace"}}>{qt.cnt}개 가능</div>
                    </div>
                  ))}
                </div>

                {/* 객관식 설정 */}
                <div id="quiz-mcq" style={{...S.card,border:"1px solid rgba(196,160,80,0.2)",marginBottom:"12px"}}>
                  <div style={{fontSize:"0.88rem",fontWeight:700,color:"#c4a050",marginBottom:"12px"}}>🎯 객관식</div>
                ...
                {/* 서술형 설정 */}
                <div id="quiz-essay" style={{...S.card,border:"1px solid rgba(100,80,200,0.2)",marginBottom:"12px"}}>
                  <div style={{fontSize:"0.88rem",fontWeight:700,color:"#9060f0",marginBottom:"6px"}}>✍️ 서술형</div>
                  <p style={{fontSize:"0.78rem",color:"rgba(255,255,255,0.45)",marginBottom:"12px"}}>{T.essaySub}</p>
                  <p style={S.settingLabel}>문제 방향</p>
                  <div style={S.optionRow}>
                ...
                {/* 변형 퀴즈 설정 */}
                {currentBook==="hebrew"&&(
                  <div id="quiz-variant" style={{...S.card,border:"1px solid rgba(80,160,120,0.2)",marginBottom:"12px"}}>
                    <div style={{fontSize:"0.88rem",fontWeight:700,color:"#50c898",marginBottom:"14px"}}>🔀 변형 퀴즈</div>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"6px"}}>
                      <p style={{...S.settingLabel,margin:0}}>변형 유형</p>
                      <button onClick={()=>setVariantCats(variantCats.length===VARIANT_CATS.length?[]:VARIANT_CATS.map(c=>c.id))} style={{...S.scrollBtn,padding:"3px 10px",fontSize:"0.72rem"}}>{variantCats.length===VARIANT_CATS.length?"전체 해제":"전체 선택"}</button>
                    </div>
                ...
                {walletDetailId!==null?(()=>{
                          ...
                          <div style={{marginBottom:"10px"}}>
                            {wlWords.map(w=>(
                              <div key={w.id} style={{display:"flex",alignItems:"center",gap:"8px",padding:"8px 0",borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
                                ...
                                <button onClick={()=>toggleWordInWallet(wl.id,w.id)} style={{padding:"2px 7px",borderRadius:"6px",background:"rgba(200,60,60,0.1)",border:"1px solid rgba(200,60,60,0.3)",color:"#f07050",cursor:"pointer",fontSize:"0.7rem"}}>제거</button>
                              </div>
                            ))}
                          </div>
                          <div style={{display:"flex",gap:"6px"}}>
                            ...
                            <button onClick={()=>listenAllStart(wlWords,wl.id)} style={{padding:"10px 12px",borderRadius:"12px",border:"1px solid rgba(196,160,80,0.35)",background:"rgba(196,160,80,0.1)",color:"#c4a050",cursor:"pointer",fontSize:"0.85rem",fontWeight:600}}>🔈</button>
                          </div>
                        </div>
                      ):(
                        <div style={S.emptyMsg}>단어가 없어요.<br/>단어 목록에서 📚 버튼을 눌러 추가하세요.</div>
                      )}
                    </div>
                  );
                })():(
                  <div>
                    ...
                    <div style={{...S.card,marginBottom:"12px"}}>
                      <div style={{fontSize:"0.82rem",fontWeight:600,color:"rgba(255,255,255,0.75)",marginBottom:"10px"}}>📚 새 단어장 만들기</div>
                      <div style={{display:"flex",gap:"8px",alignItems:"center"}}>
                        <input value={walletName} onChange={e=>setWalletName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&createWallet()} style={{...S.input,flex:1}} placeholder="단어장 이름..."/>
                        <div style={{display:"flex",gap:"4px"}}>
                        ...
                        <button onClick={createWallet} disabled={!walletName.trim()} style={{...S.btnAdd,padding:"9px 14px",opacity:walletName.trim()?1:0.4,fontSize:"0.82rem"}}>만들기</button>
                      </div>
                    </div>

                    {wallets.length===0?(
                      <div style={S.emptyMsg}>아직 단어장이 없어요.<br/>위에서 새 단어장을 만들어보세요!</div>
                    ):(
                        ...
                        {wallets.map(wl=>{
                          ...
                          return(
                            <div key={wl.id} style={{display:"flex",alignItems:"center",gap:"10px",padding:"12px 14px",borderRadius:"12px",background:"#1E1D24",border:`1px solid ${wl.color}30`}}>
                              ...
                              <button onClick={()=>setWalletDetailId(wl.id)} style={{padding:"4px 10px",borderRadius:"7px",background:`${wl.color}20`,border:`1px solid ${wl.color}40`,color:wl.color,cursor:"pointer",fontSize:"0.72rem"}}>보기</button>
                              <button onClick={()=>{if(window.confirm(`"${wl.name}"을 삭제할까요?`))deleteWallet(wl.id);}} style={{padding:"4px 9px",borderRadius:"7px",background:"rgba(200,60,60,0.1)",border:"1px solid rgba(200,60,60,0.3)",color:"#f07050",cursor:"pointer",fontSize:"0.72rem"}}>삭제</button>
                            </div>
                          );
                        })}
                      </div>
            ...
            {activeTab==="settings"&&(
              <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
                {/* 계정 */}
                <div style={S.card}>
                  <div style={{fontSize:"0.82rem",fontWeight:600,color:"rgba(255,255,255,0.75)",marginBottom:"12px"}}>👤 계정</div>
                  {user?(
                    <div>
                      ...
                      <div style={{fontSize:"0.72rem",color:"#60c880",marginBottom:"10px"}}>● 모든 기기 자동 동기화 활성화</div>
                      <button onClick={signOutUser} style={{...S.btnCancel2,width:"100%",color:"#f07050",borderColor:"rgba(200,60,60,0.3)"}}>{T.logout}</button>
                    </div>
                  ):(
                    <div>
                      <p style={{fontSize:"0.82rem",color:"rgba(255,255,255,0.5)",marginBottom:"12px",lineHeight:1.5}}>Google 계정으로 로그인하면 모든 기기에서 단어장을 동기화해요.</p>
                ...
                {/* 언어 */}
                <div style={S.card}>
                  <div style={{fontSize:"0.82rem",fontWeight:600,color:"rgba(255,255,255,0.75)",marginBottom:"12px"}}>🌐 언어</div>
                  <div style={{display:"flex",gap:"6px"}}>
                ...
                {/* 발음 */}
                <div style={S.card}>
                  <div style={{fontSize:"0.82rem",fontWeight:600,color:"rgba(255,255,255,0.75)",marginBottom:"6px"}}>🔊 발음 설정</div>
                  <div style={{fontSize:"0.72rem",color:ttsReady?"#60c880":"#f07050",marginBottom:"10px"}}>{ttsReady?"● Google TTS 연결됨":"○ 브라우저 TTS 사용 중"}</div>
                  <div style={{display:"flex",gap:"6px"}}>
                    {[{m:"auto",l:T.soundAuto,c:"#c4a050"},{m:"manual",l:T.soundManual,c:"#60c880"},{m:"mute",l:T.soundMute,c:"#9090b0"}].map(({m,l,c})=>(
        ...
        {/* ── 하단 탭바 ── */}
        {!isQuizActive&&(
          <nav style={{position:"fixed",bottom:0,left:0,right:0,background:"#17161C",borderTop:"1px solid rgba(255,255,255,0.1)",display:"flex",zIndex:200,paddingBottom:"env(safe-area-inset-bottom)"}}>
            {[
              ...
              <button key={tab.id} onClick={()=>setActiveTab(tab.id)} style={{flex:1,padding:"10px 4px 8px",border:"none",background:"transparent",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:"3px",position:"relative"}}>
                {tab.badge&&(
                  <span style={{position:"absolute",top:"7px",right:"calc(50% - 14px)",width:"14px",height:"14px",borderRadius:"50%",background:tab.id==="settings"?"#60c880":"#c4a050",fontSize:"0.52rem",fontWeight:700,color:tab.id==="settings"?"#0f1a14":"#17161C",display:"flex",alignItems:"center",justifyContent:"center",lineHeight:1}}>
